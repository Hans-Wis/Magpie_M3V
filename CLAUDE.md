...
**守則:決策必附 agent 優先序(User 裁示 2026-06-11,2026-06-11 收緊)**:**任何**需要 User 確認的選項——含技術 fork、暫停/收尾這種 meta 選擇、a/b/c 任意分支——**一律先經 ≥1 個 agent(Grok 為主,必要時 Codex/Gemini 多家)討論並排出優先順序,整合成排序清單(含理由)再讓 User 選**。不可只丟選項給 User;沒有 agent 排序前不得呈現選項。承 [[grok-standing-decision-advisor]] 決策流;agent 意見背景跑、不 block 全隊。
