#!/usr/bin/env python3
"""Compare P16 IF/cross-boundary DUT commit trace against Spike."""
...
def main() -> int:
    ...
    ok, message = compare_commits(dut, spike, label="P16 IF cross-boundary lockstep")
    (ROOT / "directed_lockstep_report.md").write_text(
        "# P16 Core IF Directed Lockstep Report\n\n"
        f"Status: {'pass' if ok else 'fail'}\n\n"
        f"Result: {message}\n\n"
        "Compared fields: `pc`, `instr`, `rd`, `wdata`.\n\n"
        "Scope: P16 IF/RV32C cross-boundary fixtures only; Spike mismatch is a failure, not a waiver.\n",
        encoding="utf-8",
    )
