...
~000021     output     [31:0] ras_top,         // 目前 stack top (給 caller 在 IF 用)
        
            // ---- Push/Pop control (synchronous) ----
 000113     input             push,            // 1 = end of this cycle push
~000018     input      [31:0] push_val,
 000013     input             pop              // 1 = end of this cycle pop
        );
        
            localparam DEPTH = 8;
            localparam PTR_BITS = 3;
        
%000006     reg [31:0]         stack [0:DEPTH-1];
 000059     reg [PTR_BITS-1:0] ptr;            // points to *next empty slot* (=stack size)
        
            // ras_top = stack[ptr-1]，empty 時 (ptr=0) 輸出 0
 000060     wire [PTR_BITS-1:0] top_idx = ptr - 3'd1;
 000104     assign ras_top = (ptr == 0) ? 32'h0 : stack[top_idx];
        
            integer i;
 000128     always @(posedge clk) begin
~000126         if (!resetn) begin
%000002             ptr <= 3'd0;
~000016             for (i = 0; i < DEPTH; i = i + 1) stack[i] <= 32'h0;
...
~000113             if (push && pop) begin
%000001                 if (ptr != 0)
%000001                     stack[top_idx] <= push_val;
                        else
%000001                     stack[0]       <= push_val;       // empty 時就視為 push 到 slot 0
                        // ptr 不變
 000111             end else if (push) begin
 000111                 stack[ptr] <= push_val;
 000111                 ptr <= ptr + 3'd1;                    // 3-bit 加法自動 wrap (覆寫底)
~000011             end else if (pop) begin
%000007                 if (ptr != 0) ptr <= ptr - 3'd1;
