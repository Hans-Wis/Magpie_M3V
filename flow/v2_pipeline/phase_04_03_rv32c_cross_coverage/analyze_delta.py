#!/usr/bin/env python3
"""Merge Phase 4.3 RV32C/cross-boundary coverage with Phase 4.2."""
...
BASE_COV = ROOT / "flow/v2_pipeline/phase_04_02_bp_ras_coverage/coverage"
BASE_INFO = BASE_COV / "coverage.info"
BASE_DAT = BASE_COV / "merged_with_phase_04_02.dat"
...
def main() -> int:
    if not BASE_INFO.exists() or not BASE_DAT.exists():
        raise SystemExit("missing Phase 4.2 base coverage artifacts")
    if not THIS_DAT.exists():
        raise SystemExit("missing Phase 4.3 coverage.dat")

    OUT_COV.mkdir(exist_ok=True)
    merged_dat = OUT_COV / "merged_with_phase_04_03.dat"
    ...
    for module in ["cdec.v", "core.v", "ifu.v"]:
    ...
    report.extend(
        [
            ...
            "## Remaining Closure",
            "",
            "Coverage is still not closed. Continue with illegal compressed/trap coverage, M-unit coverage merge/corners, "
            "residual toggle closure, and functional cover bins.",
        ]
    )
    (PHASE_DIR / "rv32c_cross_coverage_report.md").write_text("\n".join(report) + "\n", encoding="utf-8")

    cdec_delta = int(by_module["cdec.v"]["line_hit_delta"]) + int(by_module["cdec.v"]["toggle_hit_delta"])
    core_delta = int(by_module["core.v"]["line_hit_delta"]) + int(by_module["core.v"]["toggle_hit_delta"])
    if line_delta <= 0 and toggle_delta <= 0:
        print("FAIL: directed RV32C/cross test produced no DUT coverage delta")
        return 1
    if cdec_delta <= 0 and core_delta <= 0:
        print("FAIL: directed RV32C/cross test produced no cdec/core module delta")
        return 1
    print(
        "PASS: RV32C/cross directed coverage merged; "
