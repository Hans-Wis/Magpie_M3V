...
module tb_cpu_m1_fpga_top;
    ...
    cpu_m1_fpga_top #(
        ...
        .INIT_HEX("build/fpga_boot_magic.hex"),
        .BOOT_DEBUG(1'b0)
    ) dut (
        .clk(clk),
        .resetn(resetn),
    ...
    task automatic print_trace(input [31:0] pc, input [31:0] instr);
        begin
            $display("TRACE cycle=%0d pc=%08x instr=%08x magic=%08x axi_err=%0b",
                     cycle, pc, instr, dut.u_bram.mem[MAGIC_WORD], dbg_axi_err);
        end
    endtask
        ...
        while (cycle < TIMEOUT_CYC && !saw_magic) begin
            ...
            if (dbg_pc !== last_pc) begin
                if ((dbg_pc == 32'h0000_0000) ||
                    ...
                    ((dbg_pc >= RAM_BASE) && (cycle < 80))) begin
            ...
            if (dut.u_bram.mem[MAGIC_WORD] === MAGIC) saw_magic = 1'b1;
        end

        repeat (5) @(posedge clk);
        $display("RESULT magic_mem[%0d]=%08x dbg_axi_err=%0b trap=%0b saw_rom0=%0b saw_rom4=%0b saw_ram=%0b cycles=%0d",
                 MAGIC_WORD, dut.u_bram.mem[MAGIC_WORD], dbg_axi_err, trap,
                 saw_rom0, saw_rom4, saw_ram, cycle);

        if (saw_rom0 && saw_rom4 && saw_ram && saw_magic && !dbg_axi_err) begin
            $display("PASS cpu_m1_fpga_top boot ROM -> BRAM execution wrote MAGIC");
            $finish;
        end else begin
            $display("FAIL cpu_m1_fpga_top boot verification");
