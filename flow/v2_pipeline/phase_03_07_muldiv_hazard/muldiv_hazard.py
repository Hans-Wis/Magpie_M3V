#!/usr/bin/env python3
"""Compare mul/div hazard DUT commit trace against Spike."""
...
def main() -> int:
    ...
    for drow, srow in zip(dut, spike):
        raw = srow["wdata"]
        if SPIKE_BASE <= raw < SPIKE_BASE + 0x10000 and drow["wdata"] == raw - SPIKE_BASE:
            srow["wdata"] = raw - SPIKE_BASE
    write_commit_csv(ROOT / "spike_commit.trace", spike)
    ok, message = compare_commits(dut, spike, label="muldiv hazard lockstep")
    (ROOT / "muldiv_hazard_report.md").write_text(
        "# Phase 3.7 Mul/Div Hazard Lockstep Report\n\n"
        f"Status: {'pass' if ok else 'fail'}\n\n"
        f"Result: {message}\n\n"
        "Compared fields: `pc`, `instr`, `rd`, `wdata`.\n\n"
        "Scope: directed mul/div stall, result-latch, forwarding, load-use, "
        "divide-by-zero, and overflow regression. This is not random DV or "
        "coverage closure.\n",
        encoding="utf-8",
    )
