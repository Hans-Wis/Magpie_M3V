#!/usr/bin/env python3
"""Compare P18 BP/RAS directed DUT commit trace against Spike."""
...
def main() -> int:
    ...
    ok, message = compare_commits(dut, spike, label="P18 BP/RAS lockstep")
    (ROOT / "directed_lockstep_report.md").write_text(
        "# P18 Core BP/RAS Directed Lockstep Report\n\n"
        f"Status: {'pass' if ok else 'fail'}\n\n"
        f"Result: {message}\n\n"
        "Compared fields: `pc`, `instr`, `rd`, `wdata`.\n\n"
        "Scope: P18 core redirect/BP/RAS integration fixtures only; "
        "mismatch is FAIL under Spike authority.\n",
        encoding="utf-8",
    )
