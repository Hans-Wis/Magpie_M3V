...
module tb_directed_lockstep;
    ...
    always @(posedge clk) begin
        if (!resetn) begin
            mem_stall_q <= 1'b0;
            mem_wait_budget <= 2'd2;
        end else if (d_mem_valid && mem_wait_budget != 2'd0) begin
            mem_stall_q <= 1'b1;
            mem_wait_budget <= mem_wait_budget - 2'd1;
        end else begin
            mem_stall_q <= 1'b0;
        end
    ...
    always @(posedge clk) begin
        ...
        if (dut.ex_wb_valid_r && dut.ex_wb_illegal_r) begin
            ...
            if (commit_count < 40) begin
                $display("FAIL: too few commits before ebreak");
                $fatal(1);
            end
            $fclose(trace_fd);
            $display("PASS: P18 BP/RAS DUT trace wrote %0d commits before ebreak", commit_count);
            $finish;
