#!/usr/bin/env python3
"""Merge Phase 4.6 RAS recovery coverage with Phase 4.4."""
...
BASE_COV = ROOT / "flow/v2_pipeline/phase_04_04_illegal_munit_coverage/coverage"
BASE_INFO = BASE_COV / "coverage.info"
BASE_DAT = BASE_COV / "merged_with_phase_04_04.dat"
...
def main() -> int:
    if not BASE_INFO.exists() or not BASE_DAT.exists():
        raise SystemExit("missing Phase 4.4 base coverage artifacts")
    if not THIS_DAT.exists():
        raise SystemExit("missing Phase 4.6 coverage.dat")

    OUT_COV.mkdir(exist_ok=True)
    merged_dat = OUT_COV / "merged_with_phase_04_06.dat"
    ...
    report.extend(
        [
            ...
            "## Remaining Closure",
            "",
            "Coverage is still not closed. Continue with CSR high-counter/write coverage, "
            "FENCE/FENCE.I, unsigned branch decode/default triage, defensive default assertions/waivers, "
            "and functional cover bins.",
        ]
    )
    (PHASE_DIR / "ras_recovery_coverage_report.md").write_text("\n".join(report) + "\n", encoding="utf-8")

    core_line_delta = int(by_module["core.v"]["line_hit_delta"])
    ras_line_delta = int(by_module["ras.v"]["line_hit_delta"])
    if line_delta <= 0 and toggle_delta <= 0:
        print("FAIL: directed RAS test produced no DUT coverage delta")
        return 1
    if core_line_delta < 4:
        print(f"FAIL: core RAS recovery line delta too small: {core_line_delta}")
        return 1
    if ras_line_delta < 3:
        print(f"FAIL: ras pointer-edge line delta too small: {ras_line_delta}")
        return 1
    print(
        "PASS: RAS recovery coverage merged; "
