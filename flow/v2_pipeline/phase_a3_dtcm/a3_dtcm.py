#!/usr/bin/env python3
"""phase_a3_dtcm — A1 issue-decoupled MUL directed lockstep (ADR-0026 A1).
...
def main() -> int:
    ...
    ok, message = compare_commits(dut[: len(spike)], spike, label="a3_dtcm full lockstep")
    ok = ok and len(spike) >= 55          # the whole directed program must really run
    (ROOT / "a3_dtcm_report.md").write_text(
