#!/usr/bin/env python3
"""phase_a1_mul — A1 issue-decoupled MUL directed lockstep (ADR-0026 A1).
...
def main() -> int:
    ...
    ok, message = compare_commits(dut[: len(spike)], spike, label="a1_mul full lockstep")
    ok = ok and len(spike) >= 60          # the whole directed program must really run
    (ROOT / "a1_mul_report.md").write_text(
