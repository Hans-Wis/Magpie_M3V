#!/usr/bin/env python3
"""Merge Phase 4.2 BP/RAS coverage with Phase 4.1 and report DUT-only delta."""
...
BASE_COV = ROOT / "flow/v2_pipeline/phase_04_01_csr_irq_coverage/coverage"
BASE_INFO = BASE_COV / "coverage.info"
BASE_DAT = BASE_COV / "merged_with_phase_04_01.dat"
...
def main() -> int:
    if not BASE_INFO.exists() or not BASE_DAT.exists():
        raise SystemExit("missing Phase 4.1 base coverage artifacts")
    if not THIS_DAT.exists():
        raise SystemExit("missing Phase 4.2 coverage.dat")

    OUT_COV.mkdir(exist_ok=True)
    merged_dat = OUT_COV / "merged_with_phase_04_02.dat"
    ...
    for module in ["bp.v", "ras.v", "core.v"]:
    ...
    report.extend(
        [
            ...
            "## Remaining Closure",
            "",
            "Coverage is still not closed. Continue with RV32C decode corner coverage, M-unit coverage merge/corners, "
            "and functional cover bins.",
        ]
    )
    (PHASE_DIR / "bp_ras_coverage_report.md").write_text("\n".join(report) + "\n", encoding="utf-8")

    bp_delta = int(by_module["bp.v"]["line_hit_delta"]) + int(by_module["bp.v"]["toggle_hit_delta"])
    ras_delta = int(by_module["ras.v"]["line_hit_delta"]) + int(by_module["ras.v"]["toggle_hit_delta"])
    if line_delta <= 0 and toggle_delta <= 0:
        print("FAIL: directed BP/RAS test produced no DUT coverage delta")
        return 1
    if bp_delta <= 0 and ras_delta <= 0:
        print("FAIL: directed BP/RAS test produced no BP/RAS module delta")
        return 1
    print(
        "PASS: BP/RAS directed coverage merged; "
