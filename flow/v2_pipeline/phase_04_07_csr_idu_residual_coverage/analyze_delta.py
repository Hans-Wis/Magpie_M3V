#!/usr/bin/env python3
"""Merge Phase 4.7 CSR/IDU residual coverage with Phase 4.6."""
...
BASE_COV = ROOT / "flow/v2_pipeline/phase_04_06_ras_recovery_coverage/coverage"
BASE_INFO = BASE_COV / "coverage.info"
BASE_DAT = BASE_COV / "merged_with_phase_04_06.dat"
...
def _module_line(row: dict[str, str]) -> str:
    return f"{row['merged_line_hit']}/{row['line_total']} ({row['merged_line_percent']}%)"


def main() -> int:
    if not BASE_INFO.exists() or not BASE_DAT.exists():
        raise SystemExit("missing Phase 4.6 base coverage artifacts")
    if not THIS_DAT.exists():
        raise SystemExit("missing Phase 4.7 coverage.dat")

    OUT_COV.mkdir(exist_ok=True)
    merged_dat = OUT_COV / "merged_with_phase_04_07.dat"
    ...
    report.extend(
        [
            ...
            "(`alu.v` unknown ALU op, `div.v` FSM default, selected decode defaults) plus "
            "toggle and functional coverage targets. These need assertion/waiver review and "
            "functional cover bins rather than more architectural directed tests.",
        ]
    )
    (PHASE_DIR / "csr_idu_residual_coverage_report.md").write_text("\n".join(report) + "\n", encoding="utf-8")

    csr_delta = int(by_module["csr.v"]["line_hit_delta"])
    idu_delta = int(by_module["idu.v"]["line_hit_delta"])
    if line_delta <= 0 and toggle_delta <= 0:
        print("FAIL: directed CSR/IDU residual test produced no DUT coverage delta")
        return 1
    if csr_delta < 5:
        print(f"FAIL: csr line delta too small: {csr_delta}")
        return 1
    if idu_delta < 3:
        print(f"FAIL: idu line delta too small: {idu_delta}")
        return 1
    print(
        "PASS: CSR/IDU residual coverage merged; "
        ...
        f"csr={_module_line(by_module['csr.v'])}; idu={_module_line(by_module['idu.v'])}"
    )
    return 0
