#ifndef __DO_RMAHDR_
#define __DO_RMAHDR_

#ifdef __cplusplus
  extern "C" {
#endif

#define VCS_RTLIB_TLS_MODEL  __attribute__((tls_model("initial-exec")))

typedef unsigned long UP;
typedef unsigned U;
typedef unsigned char UB;
typedef unsigned short US;
typedef unsigned long long ULL;
typedef unsigned char scalar;
typedef struct vec32 vec32;
typedef unsigned char SVAL;
typedef unsigned char TYPEB;
typedef struct etype ETYPE;
typedef struct qird QIRD;
typedef unsigned char UST_e;
typedef unsigned uscope_t;
typedef U  NumLibs_t;
struct vec32 {
    U  I1;
    U  I2;
};
typedef unsigned long long InstNumType;
typedef long long InstNumSigned;
typedef InstNumType  InstNum;
typedef U  InstNum32;
typedef unsigned long RP;
typedef unsigned long RO;
typedef U  GateCount;
typedef U  NodeCount;
typedef unsigned short HsimEdge;
typedef unsigned char HsimExprChar;
typedef struct {
    U  I641;
    RP  I642;
} RmaReceiveClock1;
typedef NodeCount  FlatNodeNum;
typedef unsigned ProcessNum;
typedef unsigned long long TimeStamp64;
typedef unsigned long long TimeStamp;
typedef enum {
    PD_SING = 0,
    PD_RF = 1,
    PD_PLSE = 2,
    PD_PLSE_RF = 3,
    PD_NULL = 4
} PD_e;
typedef TimeStamp64  RmaTimeStamp64;
typedef TimeStamp  RmaTimeStamp;
typedef struct {
    scalar  I644;
    U  I645;
    RmaTimeStamp64  I646;
} MsidInfo;
typedef union {
    double I653;
    unsigned long long I654;
    unsigned  I655[2];
} rma_clock_struct;
typedef struct eblk EBLK;
typedef int (* E_fn)(void);
typedef struct eblk {
    E_fn  I662;
    struct iptmpl * I663;
    struct eblk * I665;
    unsigned I666;
    unsigned I667;
    struct eblk * I668;
    unsigned I259;
} eblk_struct;
typedef struct {
    RP  I662;
    RP  I663;
    RP  I665;
    unsigned I666;
    unsigned I667;
    RP  I668;
    unsigned I259;
} RmaEblk;
typedef struct {
    RP  I662;
    RP  I663;
    RP  I665;
    unsigned I666;
    unsigned I667;
    RP  I668;
    unsigned I259;
    unsigned val;
} RmaEblklq;
typedef struct _DEblkLaneT {
    struct _DEblkLaneT * I675;
    struct eblk * I677;
} DEblkLaneT;
typedef struct _DEblkPoolT {
    struct _DEblkLaneT * I678;
    struct _DEblkLaneT * I679;
    unsigned I680;
    unsigned I681;
} DEblkPoolT;
typedef union {
    double I653;
    unsigned long long I654;
    unsigned  I655[2];
} clock_struct;
typedef struct RmaRetain_t RmaRetain;
struct RmaRetain_t {
    RP  I714;
    RmaEblk  I661;
    ULL  I716;
    US  I717 :1;
    US  I718 :4;
    US  I237 :2;
    US  state :2;
    US  I720 :1;
    US  I721 :2;
    US  I722 :2;
    US  fHsim :1;
    US  I554 :1;
    scalar  newval;
    scalar  I725;
    RP  I726;
};
struct retain_t {
    struct retain_t * I714;
    EBLK  I661;
    ULL  I716;
    US  I717 :1;
    US  I718 :4;
    US  I237 :2;
    US  state :2;
    US  I720 :1;
    US  I721 :2;
    US  I722 :2;
    US  fHsim :1;
    US  I723 :1;
    scalar  newval;
    scalar  I725;
    void * I726;
};
typedef struct MPSched MPS;
typedef struct RmaMPSched RmaMps;
struct MPSched {
    MPS  * I703;
    scalar  I704;
    scalar  I705;
    scalar  I706;
    scalar  fHsim :1;
    scalar  I708 :1;
    scalar  I237 :6;
    U  I709;
    RP  I710;
    EBLK  I711;
    void * I712;
    UP   I713[1];
};
struct RmaMPSched {
    RP  I703;
    scalar  I704;
    scalar  I705;
    scalar  I706;
    scalar  fHsim :1;
    scalar  I708 :1;
    scalar  I237 :6;
    U  I709;
    RP  I710;
    RmaEblk  I711;
    RP  I712;
    RP   I713[1];
};
typedef struct RmaMPSchedPulse RmaMpsp;
struct RmaMPSchedPulse {
    RP  I703;
    scalar  I704;
    scalar  I705;
    scalar  I706;
    scalar  I708 :1;
    scalar  I237 :6;
    scalar  I731 :1;
    U  I709;
    RP  I710;
    RmaEblk  I711;
    scalar  I722;
    scalar  I732;
    scalar  I733;
    scalar  I734;
    U  I735;
    U  I736;
    U  I737;
    U  state;
    U  I738;
    RP  I663;
    RP  I739;
    RP  I740;
    RP   I713[1];
};
typedef struct MPItem MPI;
struct MPItem {
    U  * I742;
    void * I743;
};
typedef struct {
    RmaEblk  I661;
    RP  I744;
    scalar  I745;
    scalar  I722;
    scalar  I746;
} RmaTransEventHdr;
typedef struct RmaMPSchedPulseNewCsdf RmaMpspNewCsdf;
struct RmaMPSchedPulseNewCsdf {
    RP  I703;
    scalar  I704;
    scalar  I705;
    scalar  I706;
    scalar  fHsim :1;
    scalar  I708 :1;
    scalar  I237 :6;
    U  I709;
    RP  I710;
    RmaEblk  I711;
    scalar  I722;
    scalar  I732;
    scalar  I733;
    scalar  I734;
    U  state :4;
    U  I748 :28;
    U  I736;
    U  I737;
    RP  I749;
    RP  I663;
    RP   I713[1];
};
typedef struct red_t {
    U  I750;
    U  I751;
    U  I619;
} RED;
typedef struct predd {
    PD_e  I237;
    RED   I752[0];
} PREDD;
union rhs_value {
    vec32  I753;
    scalar  I745;
    vec32  * I722;
    double I754;
    U  I755;
};
typedef struct nbs_t {
    struct nbs_t * I756;
    struct nbs_t * I758;
    void (* I759)(struct nbs_t * I726);
    U  I760 :1;
    U  I761 :1;
    U  I762 :1;
    U  I763 :1;
    U  I764 :1;
    U  I765 :1;
    U  I766 :26;
    U  I767;
    void * I768;
    union rhs_value I769;
    vec32  I653;
    union {
        struct nbs_t * I1304;
        struct nbs_t * last;
    } I771;
} NBS;
typedef struct {
    RP  I772;
    RP  I739;
    RP  I663;
    RP  I740;
    RmaEblk  I661;
    RmaEblk  I773;
    RP  I774;
    scalar  I745;
    scalar  I722;
    char state;
    uscope_t  I775;
    U  I776;
    RP  I777;
    scalar  I732;
    scalar  I733;
    scalar  I734;
    U  I736;
    U  I737;
    RP  I712;
} RmaPulse;
typedef struct {
    RP  I772;
    RP  I739;
    RP  I663;
    RP  I740;
    RmaEblk  I661;
    RmaEblk  I773;
    RP  I774;
    scalar  I745;
    scalar  I722;
    char state;
    uscope_t  I775;
    U  I776;
    RP  I777;
    RP  I778;
    RP  I779;
    RP  I712;
    US  I748;
    RP  I780;
} RmaTransEl;
typedef enum {
    QIRDModuleC = 1,
    QIRDSVPackageC = 2,
    QIRDSpiceModuleC = 3
} QIRDModuleType;
typedef struct {
    U  I784 :1;
    U  I785 :1;
    U  I786 :1;
    U  I787 :1;
    U  I788 :1;
    U  I789 :1;
    U  I790 :1;
    U  I791 :1;
    U  I792 :1;
    U  I793 :1;
    U  I794 :1;
    U  I795 :1;
    U  I796 :1;
    U  I797 :1;
    U  I798 :1;
    U  I799 :1;
    U  I800 :1;
    U  I801 :1;
    U  I802 :1;
    U  I803 :1;
    QIRDModuleType  I804 :2;
    U  I805 :1;
    U  I806 :1;
    U  I807 :1;
    U  I808 :1;
    U  I809 :1;
    U  I810 :1;
    U  I811 :1;
    U  I812 :1;
    U  I813 :1;
    U  I814 :1;
    U  I815 :1;
    U  I816 :1;
    U  I817 :1;
    U  I818 :1;
    U  I819 :1;
    U  I820 :1;
    U  I821 :1;
    U  I822 :1;
    U  I823 :1;
    U  I824 :1;
    U  I825 :1;
    U  I826 :1;
    U  I827 :1;
    U  I828 :1;
    U  I829 :1;
    U  I830 :1;
    U  I831 :1;
    U  I832 :1;
    U  I833 :1;
    U  I834 :1;
    U  I835 :1;
    U  I836 :1;
    U  I837 :1;
    U  I838 :1;
    U  I839 :1;
    U  I840 :1;
    U  I841 :6;
} BitFlags;
typedef struct qird_dbg_flags_s {
    U  I842 :28;
    U  I843 :1;
    U  I844 :1;
    U  I845 :1;
    U  I846 :1;
} qird_dbg_flags_t;
typedef struct nlpInfoT {
    U  * I847;
    U  * I848;
    U  * I849;
    UP  * I850;
    UP  * I851;
    UP  * I852;
    UP  * I853;
    char * I854;
    UP  * I855;
    UP  * I856;
    U  * I857;
    UP  * I858;
    UP  * I859;
    UP  * I860;
    UP  * I861;
    UP  * I862;
    UP  * I863;
    UP  * I864;
    UP  * I865;
    UP  * I866;
    UP  * I867;
    UP  * I868;
    UP  * I869;
    UP  * I870;
    UP  * I871;
    char * I872;
    char * I873;
    U  * I874;
    UP  * I875;
    UP  * I876;
    UP  * I877;
    UP  * I878;
    UP  * I879;
    UP  * I880;
    UP  * I881;
    UP  * I882;
    UP  * I883;
    UP  * I884;
    UP  * I885;
    UP  * I886;
    UP  * I887;
    U  I888;
    U  I889;
    U  I890;
    U  I891;
    U  I892;
    U  I893;
    U  I894;
    U  I895;
    U  I896;
    U  I897;
    U  I898;
    U  I899;
    U  I900;
    U  I901;
    U  I902;
} nlpInfoT;
typedef struct txpInfoT {
    U  I903;
    U  I904;
    void * I905;
    void * I906;
    int (* I907)(void);
    int (* I908)(void);
    void * I909;
    void * I910;
    void * I911;
} txpInfoT;
typedef struct fsimInfoT {
    U  I912;
    U  I913;
    U  * I914;
    U  * I915;
    UP  * I916;
    UP  * I917;
    U  * I918;
    U  * I919;
    U  * I920;
    U  * I921;
} fsimInfoT;
typedef struct fgpInfoT {
    U  I922;
    U  I923;
    U  * I924;
    U  * I925;
    char * I926;
} fgpInfoT;
typedef struct peDummyModInfoT {
    char * I927;
    char * I928;
    char * I929;
} peDummyModInfoT;
typedef struct peModInfoT {
    char * I64;
    char * I930;
    char * I931;
    U  I932;
    peDummyModInfoT  * I933;
} peModInfoT;
typedef struct simProfInfoT {
    U  I934;
    U  I935;
    U  I936;
    U  * I937;
    U  * I938;
    U  * I939;
    U  * I940;
} simProfInfoT;
typedef struct hybridSimInfoT {
    U  I941;
    U  I622;
    U  I942;
    U  I943;
    U  I944;
    void * I945;
    U  * I946;
    struct hybridSimReferrerData * I947;
    struct hybridSimReferrerData * I949;
} hybridSimInfoT;
typedef struct classInfoT {
    void * I950;
    void * I951;
    void * I952;
    U  I953;
    U  I954;
    U  I955;
    U  I956;
    UP  I957;
    UP  I958;
    void * I959;
} classInfoT;
typedef struct hubbleInfoT {
    U  * I960;
    U  * I961;
    U  I962;
    U  I963;
    U  I964;
    U  I965;
    char * I966;
    U  * I967;
    U  I968;
    U  I969;
    U  * I970;
    U  I971;
    U  I972;
    U  * I973;
    U  I974;
    U  I975;
} hubbleInfoT;
struct qird {
    US  I60;
    US  I61;
    U  I62;
    U  I63;
    char * I64;
    char * I65;
    U  * I66;
    char * I67;
    char * I68;
    U  I69;
    U  I70;
    struct vcd_rt * I71;
    U  I73;
    struct _vcdOffset_rt * I74;
    U  I76;
    U  I77;
    ETYPE  * I78;
    U  I79;
    U  * I80;
    classInfoT  * I81;
    int I82;
    U  I83;
    U  * I84;
    U  * I85;
    UP  I86;
    UP  * I87;
    BitFlags  I88;
    U  I89;
    U  I90;
    U  I91;
    U  I92;
    U  * I93;
    U  I94;
    U  I95;
    U  * I96;
    U  I97;
    U  I98;
    U  I99;
    U  I100;
    U  I101;
    U  I102;
    U  I103;
    U  I104;
    U  * I105;
    U  * I106;
    U  * I107;
    U  I108;
    U  I109;
    U  * I110;
    U  I111;
    U  I112;
    U  I113;
    U  I114;
    U  * I115;
    U  I116;
    U  * I117;
    U  I118;
    U  I119;
    U  I120;
    U  I121;
    U  I122;
    U  I123;
    U  * I124;
    char * I125;
    U  I126;
    U  I127;
    U  I128;
    U  I129;
    U  * I130;
    U  I131;
    U  I132;
    U  I133;
    U  I134;
    UP  * I135;
    U  I136;
    U  I137;
    U  I138;
    U  I139;
    U  I140;
    U  I141;
    U  * I142;
    U  I143;
    U  I144;
    U  * I145;
    U  * I146;
    U  * I147;
    U  * I148;
    U  * I149;
    U  I150;
    U  I151;
    struct taskInfo * I152;
    U  I154;
    U  I155;
    U  I156;
    U  I157;
    int * I158;
    U  * I159;
    UP  * I160;
    U  * I161;
    U  I162;
    U  I163;
    int * I164;
    U  I165;
    U  I166;
    U  I167;
    U  I168;
    U  I169;
    U  I170;
    struct qrefer * I171;
    U  * I173;
    unsigned * I174;
    void * I175;
    U  I176;
    struct classStaticReferData * I177;
    U  I179;
    U  * I180;
    U  I181;
    U  * I182;
    U  I183;
    struct wakeupInfoStruct * I184;
    U  I186;
    U  I187;
    U  I188;
    U  * I189;
    U  I190;
    U  * I191;
    U  * I192;
    U  I193;
    U  I194;
    U  I195;
    U  I196;
    U  * I197;
    U  I198;
    U  I199;
    U  * I200;
    U  I201;
    U  I202;
    U  * I203;
    U  * I204;
    U  * I205;
    U  I206;
    U  I207;
    U  I208;
    U  I209;
    U  I210;
    U  I211;
    struct qdefrefee * I212;
    struct qrefee * I214;
    U  * I216;
    U  * I217;
    int (* I218)(void);
    char * I219;
    U  I220;
    U  I221;
    void * I222;
    void * I223;
    NumLibs_t  I224;
    char * I225;
    U  * I226;
    U  I227;
    U  I228;
    U  I229;
    U  I230;
    U  I231;
    U  * I232;
    U  * I233;
    int I234;
    struct clock_load * I235;
    int I250;
    struct clock_data * I251;
    int I267;
    struct clock_hiconn * I268;
    U  I272;
    U  I273;
    U  I274;
    U  I275;
    U  * I276;
    U  I277;
    U  I278;
    U  * I279;
    U  I280;
    void * I281;
    U  I282;
    U  I283;
    UP  * I284;
    void * I285;
    U  I286;
    UP  * I287;
    U  * I288;
    int (* I289)(void);
    U  * I290;
    UP  * I291;
    U  * I292;
    U  I293 :1;
    U  I294 :31;
    U  I295;
    U  I296;
    UP  * I297;
    U  * I298;
    U  I299 :1;
    U  I300 :1;
    U  I301 :1;
    U  I302 :1;
    U  I303 :28;
    U  I304;
    U  I305;
    U  I306;
    U  I307 :31;
    U  I308 :1;
    UP  * I309;
    UP  * I310;
    fsimInfoT  * I311;
    nlpInfoT  * I312;
    U  I313;
    U  I314;
    UP  * I315;
    UP  * I316;
    UP  * I317;
    UP  * I318;
    struct daidirInfo * I319;
    struct vcs_tftable * I321;
    U  I323;
    UP  * I324;
    UP  * I325;
    U  I326;
    U  I327;
    U  I328;
    UP  * I329;
    U  * I330;
    UP  * I331;
    UP  * I332;
    struct qird_hil_data * I333;
    U  I335;
    U  * I336;
    UP  (* I337)(void);
    UP  (* I338)(void);
    UP  (* I339)(void);
    UP  (* I340)(void);
    UP  (* I341)(void);
    int * I342;
    int (* I343)(void);
    char * I344;
    UP  * I345;
    UP  * I346;
    UP  (* I347)(void);
    int (* I348)(void);
    int * I349;
    int (* I350)(void);
    int (* I351)(void);
    int * I352;
    char * I353;
    U  * I354;
    U  * I355;
    U  * I356;
    U  * I357;
    void * I358;
    int (* I359)(void);
    U  I360;
    UP  * I361;
    UP  * I362;
    U  I363;
    void * I364;
    U  I365;
    U  I366;
    U  I367;
    U  I368;
    U  I369;
    U  I370;
    char * I371;
    UP  * I372;
    U  * I373;
    UP  * I374;
    U  * I375;
    U  * I376;
    U  I377 :15;
    U  I378 :14;
    U  I379 :1;
    U  I380 :1;
    U  I381 :1;
    U  I382 :3;
    U  I383 :1;
    U  I384 :1;
    U  I385 :17;
    U  I386 :3;
    U  I387 :5;
    U  I388 :1;
    U  I389 :1;
    U  I390;
    qird_dbg_flags_t  I391;
    struct scope * I392;
    U  I394;
    U  I395;
    U  I396;
    U  * I397;
    U  * I398;
    U  * I399;
    U  I400;
    U  I401;
    U  I402;
    U  I403;
    struct pcbt * I404;
    UP  * I413;
    U  I414;
    U  I415;
    U  I416;
    U  I417;
    void * I418;
    void * I419;
    void * I420;
    int I421;
    U  * I422;
    U  I423;
    int I424;
    UP  * I425;
    U  I426;
    U  I427;
    U  I428;
    U  I429;
    U  I430;
    void * I431;
    UP  * I432;
    U  I433;
    U  I434;
    void * I435;
    U  I436;
    txpInfoT  * I437;
    U  I438;
    U  I439;
    U  I440;
    U  I441;
    hubbleInfoT  * I442;
    U  I443;
    U  I444;
    char * I445;
    U  I446;
    U  * I447;
    U  I448;
    U  * I449;
    U  I450;
    U  I451;
    U  I452;
    U  I453;
    U  I454;
    U  * I455;
    char * I456;
    U  I457;
    U  I458;
    U  I459;
    simProfInfoT  * I460;
    U  I461;
    U  I462;
    UP  * I463;
    U  I464;
    U  I465;
    U  I466;
    struct cosim_info * I467;
    U  I469;
    U  * I470;
    hybridSimInfoT  * I471;
    U  * I472;
    U  I473;
    U  I474;
    U  * I475;
    U  I476;
    U  I477;
    U  * I478;
    U  I479;
    U  * I480;
    U  I481;
    U  * I482;
    U  I483;
    U  I484;
    U  * I485;
    struct mhdl_outInfo * I486;
    UP  * I488;
    U  I489;
    UP  * I490;
    U  I491;
    void * I492;
    U  * I493;
    void * I494;
    char * I495;
    U  I496;
    U  * I497;
    char * I498;
    U  I499;
    struct regInitInfo * I500;
    UP  * I502;
    U  * I503;
    char * I504;
    U  I505;
    U  I506;
    U  I507;
    U  I508;
    U  I509;
    U  I510;
    U  I511;
    U  I512;
    UP  * I513;
    U  I514;
    U  I515;
    U  I516;
    UP  * I517;
    U  I518;
    UP  * I519;
    U  I520;
    UP  * I521;
    U  I522;
    U  I523;
    U  I524;
    U  * I525;
    U  I526;
    U  I527;
    U  I528;
    U  * I529;
    U  * I530;
    UP  * I531;
    UP  * I532;
    void * I533;
    UP  I534;
    void * I535;
    void * I536;
    void * I537;
    void * I538;
    void * I539;
    UP  I540;
    U  * I541;
    U  * I542;
    void * I543;
    U  I544 :1;
    U  I545 :31;
    U  I546;
    U  I547;
    U  I548;
    int I549;
    U  I550 :1;
    U  I551 :1;
    U  I552 :1;
    U  I553 :1;
    U  I554 :28;
    void * I555;
    void * I556;
    void * I557;
    void * I558;
    void * I559;
    void * I560;
    void * I561;
    void * I562;
    UP  * I563;
    U  * I564;
    U  I565;
    char * I566;
    U  * I567;
    U  * I568;
    U  * I569;
    char * I570;
    int * I571;
    int * I572;
    UP  * I573;
    struct etype * I574;
    U  I575;
    U  I576;
    U  * I577;
    struct etype * I578;
    U  I579;
    U  I580;
    U  I581;
    U  * I582;
    void * I583;
    U  I584;
    U  I585;
    void * I586;
    fgpInfoT  * I587;
    U  I588;
    peModInfoT  * I589;
    U  I590;
    struct covreg_rt * I591;
    U  * I593;
    U  I594;
    U  I595;
    U  * I596;
    U  * I597;
    U  I598;
    U  I599;
};
typedef struct {
    int * I976;
    int * I977;
    int I978;
    union {
        long long enumDesc;
        long long classId;
    } I979;
} TypeData;
struct etype {
    U  I4 :8;
    U  I5;
    U  I6;
    U  I7 :1;
    U  I8 :1;
    U  I9 :1;
    U  I10 :1;
    U  I11 :1;
    U  I12 :1;
    U  I13 :1;
    U  I14 :1;
    U  I15 :1;
    U  I16 :4;
    U  I17 :1;
    U  I18 :1;
    U  I19 :1;
    U  I20 :1;
    U  I21 :1;
    U  I22 :1;
    U  I23 :1;
    U  I24 :1;
    U  I25 :2;
    U  I26 :1;
    U  I27 :2;
    U  I28 :1;
    U  I29 :1;
    U  I30 :1;
    U  I31 :1;
    U  I32 :1;
    U  I33 :1;
    TypeData  * I34;
    U  I35;
    U  I36;
    U  I37 :1;
    U  I38 :1;
    U  I39 :1;
    U  I40 :1;
    U  I41 :2;
    U  I42 :2;
    U  I43 :1;
    U  I44 :1;
    U  I45 :1;
    U  I46 :1;
    U  I47 :1;
    U  I48 :1;
    U  I49 :1;
    U  I50 :1;
    U  I51 :1;
    U  I52 :1;
    U  I53 :1;
    U  I54 :1;
    U  I55 :1;
    U  I56 :1;
    U  I57 :1;
    U  I58 :9;
};
typedef struct pcbt {
    U  * I406;
    UP  I81;
    U  I407;
    U  I408;
    U  I409;
    U  I410;
    U  I411;
    U  I412;
} PCBT;
struct iptmpl {
    QIRD  * I669;
    void * I670;
    UP  I671;
    UP  I672;
    UP  I673;
    struct iptmpl * I674[2];
};
typedef unsigned long long FileOffset;
typedef struct _HsCgPeriod {
    U  I1042;
    U  I1043;
} HsCgPeriod;
typedef struct {
    U   I1044[2];
    U  I1045 :1;
    U  I1046 :1;
    U  I1047 :8;
    U  I1048 :8;
    U  I1049 :8;
    U  I1050 :4;
    U  I1051 :1;
    U  I1052 :1;
    unsigned long long I1053;
    unsigned long long I1054;
    unsigned long long I1055;
    unsigned long long I1056;
    unsigned long long I1043;
    U  I1042;
    U  I1057;
    U  I1058;
    U  I1059;
    U  I1060;
    U  I1061;
    HsCgPeriod  * I1062[10];
} HsimSignalMonitor;
typedef struct {
    ULL   I1063[4];
    ULL   I1064[4];
} HsimPostNbaClkMonitor;
typedef struct {
    InstNum  I1065;
    FlatNodeNum  I1066;
    U  I997;
    scalar  I1067;
    UB  I1068;
    UB  I1069;
    UB  I1070;
    UB  I1071;
    UB  I1072;
    UB  I1073;
    ULL  I1074;
    U  I1075;
    U  I1076;
    U  I1077;
    U  I1078;
    U  I1079;
    U  I1080;
    U  I1081;
    U  I1082;
    HsimSignalMonitor  * I1083;
    HsimPostNbaClkMonitor  * I1084;
    RP  I1085;
    RmaTimeStamp64  I646;
    U  I1086;
    RmaTimeStamp64  I1087;
    U  I1088;
    UB  I1089 :1;
    UB  I1090 :1;
    UB  I1091 :1;
    ULL  I1092;
    ULL  I1093;
    ULL  I1094;
    ULL  I1095;
    ULL  I1042;
    U  I1096 :1;
    U  I1097 :1;
    U  I1098 :30;
    U  I1099;
    ULL  I1100;
    ULL  I1101;
    ULL  I1102;
    ULL  I1103;
    RP  I1104;
} HsimNodeRecord;
typedef struct {
    RP  I1112;
    RP  pcode;
} RmaIbfPcode;
typedef struct {
    RP  I1112;
    RP  I663;
} RmaIbfIp;
typedef struct {
    RP  I1112;
    RP  I663;
    U  I259;
} RmaIbfIp_RTS;
typedef struct {
    U  I759;
    InstNum32  I625;
} RmaIbfIpById;
typedef struct {
    U  I666;
    U  val;
} RmaDynamicEblklq;
typedef struct {
    U  I666;
    U  I667;
} RmaFgpDynamicEblklq;
typedef struct {
    U  val;
} RmaDynamicEblk;
typedef struct {
    scalar  I1113;
    UB  I1114;
    US  I1115;
} JaguarClockGlitch;
typedef struct {
    U  I649 :28;
    U  I1116 :1;
    U  I1117 :1;
    U  I1118 :1;
    U  I1119 :1;
    U  I1120;
    RP  I1121;
    RP  I1122;
    RP  I1123;
    U   I1124[1];
} RmaDbsedRtlLoads;
typedef struct {
    RP  I1125;
    RP  pfn;
    RP  pcode;
} RmaSeqPrimOutputEblkData;
typedef RP  RCICODE;
typedef struct {
    RmaEblk  I661;
} RmaEvTriggeredOrSyncLoadCg;
typedef struct {
    RO  I986;
    RP  pcode;
} SchedGateFanout;
typedef struct {
    RO  I986;
    RP  pcode;
    U   I1020[4];
} SchedSelectGateFanout;
typedef struct {
    RP  pcode;
    RmaEblklq  I661;
} SchedGateEblk;
typedef struct {
    RP  pcode;
    RmaEblklq  I661;
    UB  * I1126;
} SchedSelectGateEblk;
typedef struct {
    RP  pcode;
    RP  I662;
    RP  I663;
    UB  * I1126;
} SchedSelectGateCall;
typedef struct {
    RmaEblk  I661;
    RP  I1127;
} RmaAnySchedSampleSCg;
typedef struct {
    RmaEblk  I661;
    RP  I1126;
    RP  I1127;
    vec32  I1128;
} RmaAnySchedVCg;
typedef struct {
    RmaEblk  I661;
    RP  I1126;
    RP  I1127;
    vec32   I721[1];
} RmaAnySchedWCg;
typedef struct {
    RmaEblk  I661;
    RP  I1126;
    RP  I1127;
    scalar   I1129[1];
} RmaAnySchedECg;
typedef struct {
    U  I1130;
    U  I649;
    U  I997;
    U  I1131;
    RmaIbfIp  * I1132;
    EBLK  I661;
    void * val;
} RmaThreadSchedCompiledLoads;
typedef struct {
    U  I649;
    U  I657;
    RmaThreadSchedCompiledLoads  * I1133;
} RmaSchedCompileLoadsCg;
typedef RmaThreadSchedCompiledLoads  ** RmaThreadSchedCompiledLoadsArr;
typedef struct {
    U  I1134;
    U  I1135;
    RmaIbfIp  * I1136;
    RmaThreadSchedCompiledLoadsArr  I1137;
} RmaJaguarIbnSchedCg;
typedef struct {
    RP  I1138;
} RmaRootCbkCg;
typedef struct {
    RP  I1139;
} RmaRootForceCbkCg;
typedef struct {
    RmaEblk  I661;
    RP  I1140;
} RmaForceCbkJmpCg;
typedef struct {
    U  I61;
    U  I657 :31;
    U  I1141 :1;
    vec32  I753;
    U  I1142;
    RP  I1143;
    RP  I1144;
} RmaForceSelectorV;
typedef struct {
    U  I61;
    RmaIbfPcode  I1151;
    RmaIbfPcode  I1152;
} RmaOneInputPartitionGateCap;
typedef struct {
    RmaIbfPcode  I1151;
    RmaIbfPcode  I1152;
} RmaOneInputPartitionGateCapAggrCg;
typedef struct {
    U  I61;
    RmaIbfPcode  I1153;
} RmaNetTypeDriverGate;
typedef struct {
    U  I61;
    U  I603;
    RmaIbfPcode   I1153[1];
} RmaNetTypeScatterGate;
typedef struct {
    U  I61;
    RmaIbfPcode  I1153;
} RmaNetTypeGatherGate;
typedef struct {
    RmaIbfPcode  I1151;
    U  I1154 :3;
    U  I1155 :1;
    U  I1156 :1;
    U  I1157 :16;
} RmaNbaGateOfn;
typedef struct {
    U  I61;
    NBS  I1158;
    RmaIbfPcode  I1151;
} RmaNbaGate1;
typedef struct {
    RP  ptable;
    RP  pfn;
    RP  pcode;
} Rma1InputGateFaninCgS;
typedef struct RmaSeqPrimOutputS_ RmaSeqPrimOutputOnClkS;
struct RmaSeqPrimOutputS_ {
    RP  pfn;
    U  state;
    U  I1161;
    RP  I1162;
    scalar  val;
    scalar  I1163;
    U  I1164 :1;
    U  I1165 :1;
    U  I1166 :1;
    union {
        U  I641;
        U  I623;
    } I634;
    RP   I1167[1];
};
typedef struct RmaSimonFlopOutputV_ RmaSimonFlopOutputV;
typedef struct RmaSimonLatchOutputV_ RmaSimonLatchOutputV;
struct RmaSimonFlopOutputV_ {
    RP  pfn;
    RP  I1169;
    RP  I1170;
};
typedef struct RmaSimonResetFlopOutputV_ RmaSimonResetFlopOutputV;
struct RmaSimonResetFlopOutputV_ {
    RP  pfn;
    RP  I1169;
    RP  I1170;
    RP  I1174;
    scalar  I1175;
};
struct RmaSimonLatchOutputV_ {
    RP  pfn;
    RP  I1169;
    RP  I1170;
    scalar  I1172;
};
typedef struct {
    U  I61;
    U  iinput;
    UB  I1177;
    RP  I1178;
} RmaCondOptLoad;
typedef struct {
    U  I61;
    U  iinput;
    UB  I1177;
    RP  I1178;
} RmaMacroStateUpdate;
typedef struct {
    U  I61;
    U  state;
    U  I1179;
    UB  I1177;
    US  I1180;
    U  * I1181;
} RmaMacroState;
typedef struct {
    U  iinput;
    RP  I1182;
} RmaMultiInputLogicGateCg;
typedef struct {
    U  iinput;
    RP  ptable;
    RP  I1182;
} RmaSeqPrimEdgeInputCg;
typedef struct {
    RP  I1182;
} RmaSimonCellEdgeInputCg;
typedef struct {
    RmaEblk  I661;
    RP  pcode;
} RmaSched0GateCg;
typedef struct {
    RmaEblk  I661;
    RP  pcode;
    RP  pfn;
    scalar  I640;
    scalar  I1183;
} RmaUdpDeltaGateCg;
typedef struct {
    RmaEblk  I661;
    RP  pcode;
    RP  pfn;
    scalar  I1184;
} RmaSchedDeltaGateCg;
typedef struct {
    RP  pcode;
    RP  pfn;
    RmaDynamicEblk  I661;
    scalar  I1184;
    scalar  I1185;
    scalar  I1186;
    scalar  I554;
} RmaDynamicSchedDeltaGateCg;
typedef struct {
    UB  I1187;
    RP  I1188;
    RP  I1189;
} RmaPropNodeSeqLhsSCg;
typedef struct {
    RmaEblk  I661;
    RP  pcode;
    U  I997;
    U   I650[1];
} RmaBitEdgeEblk;
typedef struct {
    U  I61;
    U  I1190;
} RmaDelay;
typedef struct {
    U  I61;
    U  I1190;
    RP  I752;
    RmaEblk  I661;
    RmaIbfPcode  I1151;
} RmaGateDelay;
typedef struct {
    U  I61;
    RP  I752;
    RmaEblk  I661;
    RmaIbfPcode  I1151;
} RmaGateBehavioralDelay;
typedef struct {
    U  I61;
    U  I1190;
    union {
        RP  I1483;
        RP  I1807;
        RP  I1823;
    } I726;
    RmaIbfPcode  I1151;
} RmaMPDelay;
typedef struct {
    U  I61;
    U  I1190;
    RmaPulse  I1191;
    RmaIbfPcode  I1151;
} RmaMPPulseHybridDelay;
typedef struct {
    U  I61;
    U  I1190;
    RmaIbfPcode  I1151;
    RmaMps  I1192;
} RmaMPHybridDelay;
typedef struct {
    U  I61;
    U  I1193;
    RmaIbfPcode  I1151;
    RmaEblk  I711;
} RmaMPHybridDelayPacked;
typedef struct {
    U  I61;
    U  I1190;
    RmaIbfPcode  I1151;
    RmaMpspNewCsdf  I1194;
} RmaMPPulseDelay;
typedef struct {
    U  I61;
    U  I1190;
    RmaMpsp  I1194;
    RmaIbfPcode  I1151;
} RmaMPPulseOptHybridDelay;
typedef struct _RmaBehavioralTransportDelay {
    U  I61;
    U  I1190;
    RP  I619;
    RmaTransEventHdr  I1003;
    RP  I710;
    RmaIbfPcode  I1151;
} RmaBehavioralTransportDelayS;
typedef struct {
    U  I61;
    U  I1190;
    RP  I619;
    RmaTransEventHdr  I1003;
    RP  I710;
    RmaIbfPcode  I1151;
} RmaNtcTransDelay;
typedef struct {
    U  I61;
    U  I1190;
    RP  I619;
    RmaEblk  I661;
    RmaIbfPcode  I1151;
} RmaNtcTransMpwOptDelay;
typedef struct {
    U  I61;
    U  I1190;
    RmaEblk  I661;
    RmaIbfPcode  I1151;
} RmaNtcTransZeroDelay;
typedef struct {
    U  I61;
    U  I1190;
    RP  I1195;
    RP  I1196;
    RmaTransEventHdr  I1003;
    RP  I710;
    RmaIbfPcode  I1151;
} RmaNtcTransDelayRF;
typedef struct {
    U  I61;
    U  I1190;
    RP  I1195;
    RP  I1196;
    RmaEblk  I661;
    RmaIbfPcode  I1151;
} RmaNtcTransMpwOptDelayRF;
typedef struct {
    U  I61;
    U  I1190;
    RP  I1197;
    RmaTransEventHdr  I1003;
    RP  I710;
    RmaIbfPcode  I1151;
} RmaICTransDelay;
typedef struct {
    U  I61;
    U  I1190;
    RP  I1197;
    RmaEblk  I661;
    RmaIbfPcode  I1151;
} RmaICTransMpwOptDelay;
typedef struct {
    U  I61;
    U  I1190;
    U  I1198;
    RmaTransEl  I1199;
    RmaTransEventHdr  I1003;
    RP  I710;
    RmaIbfPcode  I1151;
    RP   I713[1];
} RmaMPTransHybridDelay;
typedef struct {
    U  I61;
    U  I1190;
    RmaEblk  I661;
    RmaIbfPcode  I1151;
} RmaICTransZeroDelay;
typedef struct {
    U  I61;
    U  I1190;
    RP  I752;
    RmaEblk  I661;
    RmaIbfPcode  I1151;
} RmaICSimpleDelay;
typedef struct {
    U  I61;
    U  I1190;
    union {
        RP  psimple;
        RP  I1807;
        RP  I1823;
    } I726;
    RmaIbfPcode  I1151;
} RmaICDelay;
typedef struct {
    U  I61;
    U  I1190;
    RP  I752;
    RmaEblk  I661;
    RmaIbfPcode  I1151;
} RmaPortDelay;
typedef struct {
    U  I61;
    U  I735;
} RmaDelays;
typedef struct {
    U  I1157;
    RP  I1200;
} RmaRtlXEdgesLoad;
typedef struct {
    U  I61;
    RmaRtlXEdgesLoad   I1200[(5)];
    void * I1201;
} RmaRtlXEdgesHdr;
typedef struct {
    U  I61;
    US  I1202;
    US  I1203 :1;
    US  I841 :15;
    RP  I1204;
    RP  I1205;
    RP  I1206;
} RmaRtlEdgeBlockHdr;
typedef struct {
    RP  I1207;
    RP  I1208;
} RemoteDbsedLoad;
typedef struct {
    UB  I1209 :2;
    UB  I1210 :2;
    UB  I1211 :1;
    UB  I1212 :1;
    UB  I1213 :1;
    UB  I1214 :1;
    US  I1215;
    US  I1216;
    US  I1217;
    int I1218;
    U  I622 :28;
    U  I1219 :1;
    U  I1220 :1;
    U  I1221 :1;
    RP  I1222;
    RP  I1223;
    RP  I1224;
    RP   I756[(5)];
    RP   I758[(5)];
    RP  I1225;
    RmaEblk  I661;
    RemoteDbsedLoad   I1226[1];
} RmaRtlEdgeBlock;
typedef struct TableAssign_ {
    struct TableAssign_ * I989;
    struct TableAssign_ * I744;
    U  I61;
    U  I1228 :1;
    U  I1229 :2;
    U  I1230 :1;
    U  I641 :8;
    U  I1231 :1;
    U  I1232 :1;
    U  I1233 :1;
    U  I1234 :1;
    U  I1235 :1;
    U  I1236 :1;
    U  I1237 :1;
    U  I1238 :1;
    U  I1239 :1;
    U  I1240 :1;
    U  I1241 :1;
    U  I841 :9;
    RP  ptable;
    RP  I1182;
} TableAssign;
typedef struct TableAssignFullDbs_ {
    struct TableAssignFullDbs_ * I989;
    struct TableAssignFullDbs_ * I1243;
    struct TableAssignFullDbs_ * I1244;
    struct TableAssignFullDbs_ * I1245;
    struct TableAssignFullDbs_ * I1246;
    struct TableAssignFullDbs_ * I744;
    U  I61;
    U  I1228 :1;
    U  I1229 :2;
    U  I1230 :1;
    U  I641 :8;
    U  I1231 :1;
    U  I1232 :1;
    U  I1233 :1;
    U  I1234 :1;
    U  I1235 :1;
    U  I1236 :1;
    U  I1237 :1;
    U  I1238 :1;
    U  I1239 :1;
    U  I1241 :5;
    U  I841 :6;
    RP  ptable;
    RP  I1182;
} TableAssignFullDbs;
typedef struct TableAssignLayoutOnClk_ {
    struct TableAssignLayoutOnClk_ * I989;
    struct TableAssignLayoutOnClk_ * I744;
    U  I61;
    U  I1228 :1;
    U  I1229 :2;
    U  I1230 :1;
    U  I641 :8;
    U  I1231 :1;
    U  I1232 :1;
    U  I1233 :1;
    U  I1234 :1;
    U  I1235 :1;
    U  I1236 :1;
    U  I1237 :1;
    U  I1238 :1;
    U  I1239 :1;
    U  I1240 :1;
    U  I1241 :1;
    U  I841 :9;
    RP  ptable;
    RmaSeqPrimOutputOnClkS  I1248;
    RmaEblk  I661;
} TableAssignLayoutOnClk;
typedef struct TableAssignLayoutOnClkFullDbs_ {
    struct TableAssignLayoutOnClkFullDbs_ * I989;
    struct TableAssignLayoutOnClkFullDbs_ * I1243;
    struct TableAssignLayoutOnClkFullDbs_ * I1244;
    struct TableAssignLayoutOnClkFullDbs_ * I1245;
    struct TableAssignLayoutOnClkFullDbs_ * I1246;
    struct TableAssignLayoutOnClkFullDbs_ * I744;
    U  I61;
    U  I1228 :1;
    U  I1229 :2;
    U  I1230 :1;
    U  I641 :8;
    U  I1231 :1;
    U  I1232 :1;
    U  I1233 :1;
    U  I1234 :1;
    U  I1235 :1;
    U  I1236 :1;
    U  I1237 :1;
    U  I1238 :1;
    U  I1239 :1;
    U  I1241 :5;
    U  I841 :6;
    RP  ptable;
    RmaSeqPrimOutputOnClkS  I1248;
    RmaEblk  I661;
} TableAssignLayoutOnClkFullDbs;
typedef struct {
    U  state;
    U  I1250;
} RmaSeqPrimOutputOnClkOpt;
typedef struct TableAssignLayoutOnClkOpt_ {
    struct TableAssignLayoutOnClkOpt_ * I989;
    struct TableAssignLayoutOnClkOpt_ * I744;
    U  I731;
    U  I1228 :1;
    U  I1229 :2;
    U  I1230 :1;
    U  I641 :8;
    U  I1231 :1;
    U  I1232 :1;
    U  I1233 :1;
    U  I1234 :1;
    U  I1235 :1;
    U  I1236 :1;
    U  I1237 :1;
    U  I1238 :1;
    U  I1239 :1;
    U  I1240 :1;
    U  I1241 :1;
    U  I841 :9;
    union {
        U  I1253;
        RP  ptable;
    } table;
    RmaSeqPrimOutputOnClkOpt  I1248;
    RmaSeqPrimOutputEblkData  I1254;
} TableAssignLayoutOnClkOpt;
typedef struct TableAssignLayoutOnClkOptFullDbs_ {
    struct TableAssignLayoutOnClkOptFullDbs_ * I989;
    struct TableAssignLayoutOnClkOptFullDbs_ * I1243;
    struct TableAssignLayoutOnClkOptFullDbs_ * I1244;
    struct TableAssignLayoutOnClkOptFullDbs_ * I1245;
    struct TableAssignLayoutOnClkOptFullDbs_ * I1246;
    struct TableAssignLayoutOnClkOptFullDbs_ * I744;
    U  I731;
    U  I1228 :1;
    U  I1229 :2;
    U  I1230 :1;
    U  I641 :8;
    U  I1231 :1;
    U  I1232 :1;
    U  I1233 :1;
    U  I1234 :1;
    U  I1235 :1;
    U  I1236 :1;
    U  I1237 :1;
    U  I1238 :1;
    U  I1239 :1;
    U  I1241 :5;
    U  I841 :6;
    union {
        U  I1253;
        RP  ptable;
    } table;
    RmaSeqPrimOutputOnClkOpt  I1248;
    RmaSeqPrimOutputEblkData  I1254;
} TableAssignLayoutOnClkOptFullDbs;
typedef struct {
    U  I61;
    RP  I744;
    RP  I1256;
} RmaTableAssignList;
typedef struct {
    U  I61;
    U  I1257;
    RP  I744;
    RP  I1258;
    RP  I1259;
    RP  I1260;
    RP  I1261;
    RP  I1256;
} RmaTableAssignFullDbsList;
typedef struct {
    U  I61;
    RP  I744;
    RP  I1256;
    RP  I1262;
    RP  I1162;
    RP  I1263;
    US  I641;
    UB  I1067;
    UB  I1264;
    UB  I1265;
    UB  I717;
    RP   I1266[0];
} RmaThreadTableAssignList;
typedef struct {
    RP  I1262;
    RP  I1162;
    RP  I1263;
    U  I1267 :1;
    U  I1123 :8;
    U  I1268 :8;
    U  I1269 :1;
    U  I1270 :1;
    U  I1271 :2;
    U  I1272 :1;
    U  I841 :10;
    US  I641;
    UB  I1067;
    UB  I1264;
    UB  I1265;
    UB  I717;
} RmaThreadTableHeader;
typedef struct {
    RP  I1207;
} RmaWakeupListCg;
typedef struct {
    RP  I1207;
} RmaWakeupArrayCg;
typedef struct {
    RP  I1207;
    RP  I1273;
} RmaPreCheckWakeupListCg;
typedef struct {
    RP  I1207;
    RP  I1273;
} RmaPreCheckWakeupArrayCg;
typedef struct {
    U  I1274;
    U  I641;
    RmaTimeStamp   I1275[1];
} RmaTsArray;
typedef struct {
    U  iinput;
    RP  I1276;
} RmaConditionsMdb;
typedef struct {
    RP  I1277;
    RP  I1278;
    U  I1279;
} RmaTcListHeader;
typedef struct {
    RP  I989;
    RP  I1280;
    RP  I1281;
    RP  I656;
    U  I1282;
    scalar  I1157;
    scalar  I1283;
    US  I1284 :1;
    US  I1285 :1;
    US  I1286 :1;
    US  I1287 :1;
    US  I1288 :1;
    US  I1289 :1;
    US  I1290 :1;
    US  I1291 :5;
} RmaTcCoreSimple;
typedef struct {
    RP  I989;
    RP  I1280;
    RP  I1281;
    RP  I656;
    U  I1282;
    scalar  I1157;
    scalar  I1283;
    US  I1284 :1;
    US  I1285 :1;
    US  I1286 :1;
    US  I1287 :1;
    US  I1288 :1;
    US  I1289 :1;
    US  I1290 :1;
    US  I1291 :5;
    RP  I1292;
} RmaTcCoreConditional;
typedef struct {
    RP  I989;
    RP  I1280;
    RP  I1281;
    RP  I656;
    U  I1282;
    scalar  I1157;
    scalar  I1283;
    US  I1284 :1;
    US  I1285 :1;
    US  I1286 :1;
    US  I1287 :1;
    US  I1288 :1;
    US  I1289 :1;
    US  I1290 :1;
    US  I1291 :5;
    RP  I1292;
    RP  I1293;
} RmaTcCoreConditionalOpt;
typedef struct {
    RP  I989;
    RP  I1280;
    RP  I1281;
    RP  I656;
    U  I1282;
    scalar  I1157;
    scalar  I1283;
    US  I1284 :1;
    US  I1285 :1;
    US  I1286 :1;
    US  I1287 :1;
    US  I1288 :1;
    US  I1289 :1;
    US  I1290 :1;
    US  I1291 :5;
    RP  I1293;
    RP  I1294;
    U  I1295;
    RmaConditionsMdb   arr[1];
} RmaTcCoreConditionalMtc;
typedef struct {
    RP  I1281;
    RP  I656;
    U  I1282;
    scalar  I1157;
    scalar  I1283;
    US  I1284 :1;
    US  I1285 :1;
    US  I1286 :1;
    US  I1287 :1;
    US  I1288 :1;
    US  I1289 :1;
    US  I1290 :1;
    US  I1291 :5;
} RmaTcCoreSimpleNoList;
typedef struct {
    RP  I1281;
    RP  I656;
    U  I1282;
    scalar  I1157;
    scalar  I1283;
    US  I1284 :1;
    US  I1285 :1;
    US  I1286 :1;
    US  I1287 :1;
    US  I1288 :1;
    US  I1289 :1;
    US  I1290 :1;
    US  I1291 :5;
    RP  I1292;
} RmaTcCoreConditionalNoList;
typedef struct {
    RP  I1281;
    RP  I656;
    U  I1282;
    scalar  I1157;
    scalar  I1283;
    US  I1284 :1;
    US  I1285 :1;
    US  I1286 :1;
    US  I1287 :1;
    US  I1288 :1;
    US  I1289 :1;
    US  I1290 :1;
    US  I1291 :5;
    RP  I1292;
    RP  I1293;
} RmaTcCoreConditionalOptNoList;
typedef struct {
    RP  I1281;
    RP  I656;
    U  I1282;
    scalar  I1157;
    scalar  I1283;
    US  I1284 :1;
    US  I1285 :1;
    US  I1286 :1;
    US  I1287 :1;
    US  I1288 :1;
    US  I1289 :1;
    US  I1290 :1;
    US  I1291 :5;
    RP  I1293;
    RP  I1294;
    U  I1295;
    RmaConditionsMdb   arr[1];
} RmaTcCoreConditionalMtcNoList;
typedef struct {
    RP  I1281;
    RP  I656;
    U  I1282;
    scalar  I1157;
    scalar  I1283;
    US  I1284 :1;
    US  I1285 :1;
    US  I1286 :1;
    US  I1287 :1;
    US  I1288 :1;
    US  I1289 :1;
    US  I1290 :1;
    US  I1291 :5;
    RP  I1293;
    RP  I1294;
    RP  I1167;
    U  I1295;
    RmaConditionsMdb   arr[1];
} RmaTcCoreConditionalMtcNoListMdb;
typedef struct {
    RP  I1281;
    RP  I656;
    U  I1282;
    scalar  I1157;
    scalar  I1283;
    US  I1284 :1;
    US  I1285 :1;
    US  I1286 :1;
    US  I1287 :1;
    US  I1288 :1;
    US  I1289 :1;
    US  I1290 :1;
    US  I1291 :5;
    RP  I1292;
    RP  I1167;
} RmaTcCoreConditionalNoListMdb;
typedef struct {
    RP  I1281;
    RP  I656;
    U  I1282;
    scalar  I1157;
    scalar  I1283;
    US  I1284 :1;
    US  I1285 :1;
    US  I1286 :1;
    US  I1287 :1;
    US  I1288 :1;
    US  I1289 :1;
    US  I1290 :1;
    US  I1291 :5;
    U  I1297;
    RP  I1298;
    RP  I1299;
    RP  I1292;
    RP  I1300;
    RP  I1301;
    RmaTimeStamp  I1302;
} RmaTcCoreNochange;
typedef struct {
    RP  I1303;
    RP  I989;
} RmaTcCoreNochangeList;
typedef struct {
    RP  I1281;
    RP  I656;
    U  I1282;
    scalar  I1157;
    scalar  I1283;
    US  I1284 :1;
    US  I1285 :1;
    US  I1286 :1;
    US  I1287 :1;
    US  I1288 :1;
    US  I1289 :1;
    US  I1290 :1;
    US  I1291 :5;
    RP  I1292;
    RmaEblk  I661;
    U  I1304;
} RmaTcCoreFullskew;
typedef struct {
    RP  I1305;
    RP  I989;
} RmaTcCoreFullskewList;
typedef struct {
    RP  I1276;
    RmaTimeStamp  I1306;
    scalar  I1307;
    scalar  I1308 :1;
    scalar  I731 :7;
} RmaConditionalTSLoadNoList;
typedef struct {
    RP  I989;
    RP  I1276;
    RmaTimeStamp  I1306;
    scalar  I1307;
    scalar  I1308 :1;
    scalar  I731 :7;
} RmaConditionalTSLoad;
typedef struct {
    RmaTimeStamp  I1306;
    scalar  I1307;
    US  I1157;
    RP  I1293;
} RmaConditionalTSLoadOptNoList;
typedef struct {
    RP  I989;
    RmaTimeStamp  I1306;
    scalar  I1307;
    US  I1157;
    RP  I1293;
} RmaConditionalTSLoadOpt;
typedef struct {
    RP  I1293;
    RP  I1309;
    U  I1295;
    RmaConditionsMdb   arr[1];
} RmaConditionalTSLoadMtcNoList;
typedef struct {
    RP  I1167;
    RP  I1293;
    RP  I1309;
    U  I1295;
    RmaConditionsMdb   arr[1];
} RmaConditionalTSLoadMtcNoListMdb;
typedef struct {
    RP  I989;
    RP  I1293;
    RP  I1309;
    U  I1295;
    RmaConditionsMdb   arr[1];
} RmaConditionalTSLoadMtc;
typedef struct {
    U  I1310;
    U  I1311;
    FlatNodeNum  I1111;
    U  I997;
    U  I1312;
    U  I1313;
    RmaIbfPcode  I1151;
    union {
        scalar  I1314;
        vec32  I1315;
        scalar  * I1316;
        vec32  * I1317;
    } val;
} RmaScanSwitchData;
typedef struct {
    U  I1310;
    U  I1311;
    U  I997;
    U  I1318;
    FlatNodeNum  I1111;
    RmaIbfPcode  I1151;
    union {
        scalar  I1314;
        vec32  I1315;
        scalar  * I1316;
        vec32  * I1317;
        double I1319;
        scalar  * I1320;
    } val;
} RmaRtvsGateData;
typedef struct {
    RP  I989;
    RP  I744;
    RP  I1321;
} RmaDoublyLinkedListElem;
typedef struct {
    RP  I1322;
    RP  I1323;
    U  I1324 :1;
    U  finterpreted :1;
    U  I1326 :1;
    U  I1327 :4;
    U  I1328 :1;
    U  I1091 :1;
    U  I1329 :1;
    U  I1330;
} RmaSwitchGateInCbkListInfo;
typedef struct {
    union {
        RmaDoublyLinkedListElem  I1879;
        RmaSwitchGateInCbkListInfo  I2;
    } I634;
    RmaIbfPcode  I1151;
} RmaSwitchGate;
typedef struct RmaNonEdgeLoadData1_ {
    US  I1331;
    scalar  val;
    scalar  I1332 :1;
    scalar  I1333 :1;
    scalar  I1334 :3;
    scalar  I1335 :1;
    scalar  I1336 :1;
    scalar  I1337 :1;
    U  I1338;
    RO  I1339;
    RP  I756;
    RP  I1340;
    RP  I1111;
    RP  I1341;
    RP  I1342;
} RmaNonEdgeLoadData1;
typedef struct RmaNonEdgeLoadHdr1_ {
    UB  I1332;
    UB  I1343;
    US  I1067;
    RmaNonEdgeLoadData1  * I1200;
    RmaNonEdgeLoadData1  * I744;
    RP  I1225;
    void * I1201;
} RmaNonEdgeLoadHdr1;
typedef struct RmaNonEdgeLoadHdrPrl1_ {
    U  I1344;
    U  I1345 :1;
    RP  I656;
} RmaNonEdgeLoadHdrPrl1;
typedef struct RmaNonEdgeLoadArrayHdr1_ {
    UB  I1332 :1;
    UB  I1343 :1;
    UB  I1067;
    int I622;
    RmaNonEdgeLoadData1  * I626;
    U  * I1346;
    RP  I1225;
    void * I1201;
} RmaNonEdgeLoadArrayHdr1;
typedef struct RmaNonEdgeLoadDataArray1_ {
    U  I1347;
} RmaNonEdgeLoadDataArray1;
typedef struct RmaChildClockProp_ {
    RP  I756;
    RP  I1348;
    RP  I1111;
    RP  pcode;
    scalar  val;
} RmaChildClockProp;
typedef struct RmaChildClockPropList1_ {
    RmaChildClockProp  * I1200;
    RmaChildClockProp  * I744;
} RmaChildClockPropList1;
typedef struct {
    UB  I1349;
    UB  I1350 :1;
    UB  I1351 :1;
    UB  I1352 :1;
    UB  I1353 :1;
    UB  I841 :4;
    US  cedges;
    RP  I1354;
} RmaMasterXpropLoadHdr;
typedef struct {
    UB  I1349;
    UB  I1355;
    UB  I1356;
    UB  I1357 :1;
    UB  I1358 :1;
    UB  I841 :6;
    U  cedges;
} RmaXpropLoadHdr;
typedef struct {
    UB  I1355;
    UB  I1356;
    UB  I1359;
    UB  I1360;
    U  cedges :29;
    U  I1351 :1;
    U  I1361 :1;
    U  I1362 :1;
    U  I1363;
    U  I1364;
    RP  I1365;
    RP  I1366;
    RmaRtlEdgeBlockHdr  * I1367;
} RmaChildXpropLoadHdr;
struct clock_load {
    U  I237 :5;
    U  I238 :12;
    U  I239 :1;
    U  I240 :2;
    U  I241 :1;
    U  I242 :1;
    U  I243 :1;
    U  I244 :9;
    U  I245;
    U  I246;
    void (* pfn)(void * I248, char val);
};
typedef struct clock_data {
    U  I253 :1;
    U  I254 :1;
    U  I255 :1;
    U  I256 :1;
    U  I237 :5;
    U  I238 :12;
    U  I257 :6;
    U  I258 :1;
    U  I240 :2;
    U  I241 :1;
    U  I244 :1;
    U  I259;
    U  I260;
    U  I261;
    U  I245;
    U  I262;
    U  I263;
    U  I264;
    U  I265;
    U  I266;
} HdbsClockData;
struct clock_hiconn {
    U  I270;
    U  I271;
    U  I245;
    U  I240;
};
typedef union _RmaCbkMemOptUnion {
    RP  I1368;
    RP  I1369;
    RP  I1370;
} RmaCbkMemOptUnion;
typedef struct _RmaDaiOptCg {
    RmaCbkMemOptUnion  I1371;
} RmaDaiOptCg;
struct futq_slot2 {
    U  I701;
    U   I702[32];
};
struct futq_slot1 {
    U  I698;
    struct futq_slot2  I699[32];
};
struct futq_info {
    scalar  * I693;
    U  I694;
    U  I695;
    struct futq_slot1  I696[32];
};
struct futq {
    struct futq * I682;
    struct futq * I684;
    RmaEblk  * I685;
    RmaEblk  * I686;
    U  I666;
    U  I1;
    DEblkPoolT  * I687;
};
struct sched_table {
    struct futq * I688;
    struct futq I689;
    struct hash_bucket * I690;
    struct hash_bucket * I692;
};
struct dummyq_struct {
    clock_struct  I1372;
    EBLK  * I1373;
    EBLK  * I1374;
    EBLK  * I1375;
    struct futq * I1376;
    struct futq * I1377;
    struct futq * I1378;
    struct sched_table * I1379;
    struct futq_info * I1381;
    struct futq_info * I1383;
    U  I1384;
    U  I1385;
    U  I1386;
    U  I1387;
    U  I1388;
    U  I1389;
    U  I1390;
    struct millenium * I1391;
    EBLK  * I1393;
    EBLK  * I1394;
    EBLK  * I1395;
    EBLK  * I1396;
    EBLK  * I1397;
    EBLK  * I1398;
    EBLK  * I1399;
    EBLK  * I1400;
    EBLK  * I1401;
    EBLK  * I1402;
    EBLK  * I1403;
    EBLK  * I1404;
    EBLK  * I1405;
    EBLK  * I1406;
    EBLK  * I1407;
    EBLK  * I1408;
    EBLK  * I1409;
    MPS  * I1410;
    struct retain_t * I1411;
    EBLK  * I1412;
    EBLK  * I1413;
    EBLK  * I1414;
    EBLK  * I1415;
    EBLK  * I1416;
    EBLK  * I1417;
    EBLK  * I1418;
    EBLK  * I1419;
    EBLK  * I1420;
    EBLK  * I1421;
    EBLK  * I1422;
    EBLK  * I1423;
    EBLK  * I1424;
    EBLK  * I1425;
    EBLK  * I1426;
    EBLK  * I1427;
    EBLK  * I1428;
    EBLK  * I1429;
    EBLK  * I1430;
    EBLK  * I1431;
    EBLK  * I1432;
    EBLK  * I1433;
    EBLK  * I1434;
    EBLK  * I1435;
    EBLK  * I1436;
    EBLK  * I1437;
    EBLK  I1438;
    EBLK  * I1439;
    EBLK  * I1440;
    int I1441;
    struct vcs_globals_t * I1442;
    clock_struct  I1444;
    unsigned long long I1445;
    EBLK  * I1446;
    EBLK  * I1447;
    void * I1448;
    U  semilerOptQueuesFlag;
    U  I1450;
};
typedef void (* FP)(void *  , scalar   );
typedef void (* FP1)(void *  );
typedef void (* FPRAP)(void *  , vec32  *  , U   );
typedef U  (* FPU1)(void *  );
typedef void (* FPV)(void *  , UB  *  );
typedef void (* FPVU)(void *  , UB  *  , U   );
typedef void (* FPLSEL)(void *  , scalar   , U   );
typedef void (* FPLSELV)(void *  , vec32  *  , U   , U   );
typedef void (* FPFPV)(UB  *  , UB  *  , U   , U   , U   , U   , U   , UB  *  , U   );
typedef void (* FPFA)(UB  *  , UB  *  , U   , U   , U   , U   , U   , U   , UB  *  , U   );
typedef void (* FPRPV)(UB  *  , U   , U   , U   );
typedef void (* FPEVCDLSEL)(void *  , scalar   , U   , UB  *  );
typedef void (* FPEVCDLSELV)(void *  , vec32  *  , U   , U   , UB  *  );
typedef void (* FPNTYPE_L)(void *  , void *  , U   , U   , UB  *  , UB  *  , UB  *  , UB  *  , UB  *  , UB  *  , UB  *  , U   );
typedef void (* FPNTYPE_H)(void *  , void *  , U   , U   , UB  *  , UB  *  , UB  *  , UB  *  , U   );
typedef void (* FPNTYPE_LPAP)(void *  , void *  , void *  , U   , U   , UB  *  , UB  *  , U   );
typedef void (* FPNTYPE_HPAP)(void *  , void *  , void *  , U   , U   , UB  *  , UB  *  , UB  *  , UB  *  , U   );
typedef struct _lqueue {
    int I1451;
    struct _lqueue * I714;
    EBLK  * I1453;
    EBLK  * I665;
    DEblkPoolT  * I687;
} Queue;
typedef struct {
    void * I1454;
    void * I1455;
    void * I1456;
    void * I1457;
    void * I1458[2];
    void * I1459;
} ClkLevel;
typedef struct {
    EBLK  * I1460;
    EBLK  * I1461;
} EBlkList;
typedef struct {
    unsigned long long I1462;
    EBLK  I1361;
    U  I1463;
    U  I1464;
    union {
        void * pHeap;
        Queue  * pList;
    } I634;
    unsigned long long I1465;
    ClkLevel  I1466;
    EBLK  * I1467;
    EBlkList  I1468;
    EBlkList  I1469;
    Queue   I1470[1];
} Qhdr;
extern UB   Xvalchg[];
extern UB   X4val[];
extern UB   X3val[];
extern UB   X2val[];
extern UB   XcvtstrTR[];
extern UB   Xcvtstr[];
extern UB   Xbuf[];
extern UB   Xbitnot[];
extern UB   Xwor[];
extern UB   Xwand[];
extern U   Xbitnot4val[];
extern UB   globalTable1Input[];
extern __thread unsigned long long vcs_clocks;
extern __thread unsigned long long ref_vcs_clocks;
extern UB   Xunion[];
extern UB   vcsXunionCg[];
extern UB   vcsXworCg[];
extern UB   vcsXwandCg[];
extern U  fRTFrcRelCbk;
extern FP  txpFnPtr;
extern UP   bepatsFunctionArray[];
extern FP   rmaFunctionArray[];
extern UP   rmaFunctionRtlArray[];
extern UP   rmaFunctionHsRtlArray[];
extern FP   rmaFunctionLRArray[];
extern U  rmaFunctionCount;
extern U  rmaFunctionLRCount;
extern U  rmaFunctionLRDummyCount;
extern UP  rmaFunctionDummyEndPtr;
extern FP   rmaFunctionFanoutArray[];
extern __thread UB  dummyScalar;
extern __thread UB  fScalarIsForced;
extern __thread UB  fScalarIsReleased;
extern __thread UB  fScalarIsDeposited;
extern __thread UB  fForcedportSemanticsActive;
extern U  fNotimingchecks;
extern U  fFsdbDumpOn;
extern RP  * iparr;
extern FP1  * rmaPostAnySchedFnPtr;
extern FP1  * rmaPostAnySchedFnSamplePtr;
extern FP1  * rmaPostAnySchedVFnPtr;
extern FP1  * rmaPostAnySchedWFnPtr;
extern FP1  * rmaPostAnySchedEFnPtr;
extern FP1  * rmaPostSchedUpdateClockStatusFnPtr;
extern FP1  * rmaPostSchedUpdateClockStatusNonCongruentFnPtr;
extern FP1  * rmaPostSchedUpdateEvTrigFnPtr;
extern FP1  * rmaSched0UpdateEvTrigFnPtr;
extern FP1  * rmaPostSchedRecoveryResetDbsFnPtr;
extern U  fGblDataOrTime0Prop;
extern UB   rmaEdgeStatusValArr[];
extern UB   rmaposEdgeStatusArray[];
extern UB   rmanegEdgeStatusArray[];
extern FP1  * propForceCbkSPostSchedCgFnPtr;
extern FP1  * propForceSPostSchedCgFnPtr;
extern FP1  * propForceSPostSchedNativeVcCgFnPtr;
extern U  VCS_NCBK_Enabled;
extern UB  * ptableGbl;
extern U  * vcs_ptableOffsetsGbl;
extern UB  * expandedClkValues;
extern __thread Qhdr  * lvlQueue;
extern __thread unsigned threadIndex;
extern __thread unsigned fBufferingEvent;
extern int cPeblkThreads;
extern US   xedges[];
extern U  mhdl_delta_count;
extern U  ignoreSchedForScanOpt;
extern U  fignoreSchedForDeadComboCloud;
extern int fZeroUser;
extern U  fEveBusPullVal;
extern U  fEveBusPullFlag;
extern U  fFutEventPRL;
extern U  fParallelEBLK;
extern __thread UB  fNettypeIsForced;
extern __thread UB  fNettypeIsReleased;
extern EBLK  * peblkFutQ1Head;
extern EBLK  * peblkFutQ1Tail;
extern US  * edgeActionT;
extern unsigned long long * derivedClk;
extern U  fHashTableSize;
extern U  fSkipStrChangeOnDelay;
extern U  fHsimRuntimeSdfOpt;
extern U  fSkipMpTsUpdateOnStr;
extern scalar   edgeChangeLookUp[4][4];
extern U  fDoingTime0Prop;
extern U  fLoopDetectMode;
extern int gFLoopDectCodeEna;
extern U  fLoopReportRT;
extern U  fIsCalledFromWaveform;
extern U  rmaProfEvtProp;
extern U  semilerOptQueuesFlag;
extern int semilerOpt;


extern void *mempcpy(void* s1, void* s2, unsigned n);
extern RP* rmaGetDynamicVcdLocation(U archiveId, U patIdx, UB* pupdate);
extern UB* rmaEvalDelays(UB* pcode, scalar val, scalar *preVal, void* pwaveform, U* wf_cloads, U isPortOrICDelayEvalSkipReqd);
extern UB* rmaEvalDelaysV(UB* pcode, vec32* pval);
extern void rmaPopTransEvent(UB* pcode);
extern void rmaSetupFuncArray(UP* ra, U c, U w);
extern void rmaSetupRTLoopReportPtrs(UP* funcs, UP* rtlFuncs, U cnt, U cntDummy, UP end);
extern void SinitHsimPats(void);
extern void VVrpDaicb(void* ip, U nIndex);
extern int SDaicb(void *ip, U nIndex);
extern void SDaicbForHsimNoFlagScalar(void* pDaiCb, unsigned char value, unsigned char isStrength);
extern void SDaicbForHsimWithFlagScalar(void* pDaiCb, unsigned char value, unsigned char isStrength);
extern void SDaicbForHsimNoFlagDynElabScalar(U* mem, unsigned char value, unsigned char isStrength);
extern void SDaicbForHsimWithFlagDynElabScalar(U* mem, unsigned char value, unsigned char isStrength);
extern void SDaicbForHsimNoFlagDynElabFrcRel(U* mem, unsigned char reason, int msb, int lsb, int ndx);
extern void SDaicbForHsimNoFlagFrcRel(void* pDaiCb, unsigned char reason, int msb, int lsb, int ndx);
extern void hsimDispatchForVcd(RP p, U val);
extern void* hsimGetCbkMemOptCallback(RP p);
extern void hsimDispatchNoDynElabS(RP* p, U val, U isStrength);
extern void hsimDispatchDANoDynElabS(RP* p, U val, UB* da_byte, U isStrength);
extern U slaveTogglesThisTime;
extern void* hsimGetCbkPtrNoDynElab(RP p);
extern void hsimDispatchDynElabS(U** pvcdarr, U** pcbkarr, U val, U isScalForced, U isScalReleased, U isStrength);
extern void hsimDispatchNoDynElabVector(RP* /*RmaDaiOptCg* */p, void* pval, U /*RmaValueType*/ vt, U cbits);
extern void copyAndPropRootCbkCgS(RmaRootCbkCg* pRootCbk, scalar val);
extern void copyAndPropRootCbkCgV(RmaRootCbkCg* rootCbk, vec32* pval );
extern void copyAndPropRootCbkCgW(RmaRootCbkCg* rootCbk, vec32* pval );
extern void copyAndPropRootCbkCgE(RmaRootCbkCg* rootCbk, scalar* pval );
extern void Wsvvar_callback_non_dynamic1(RP* ptr, int);
extern void rmaExecEvSyncList(RP plist);
extern void Wsvvar_callback_virt_intf(RP* ptr);
extern void Wsvvar_callback_hsim_var(RP* ptr);
extern void checkAndConvertVec32To2State(vec32* value, vec32* svalue, U cbits, U* pforcedBits);
extern unsigned int fGblDataOrTime0Prop;
extern void SchedSemiLerMP1(UB* pmps, U partId);
extern void SchedSemiLerMPO(UB* pmpso, U partId);
extern void rmaDummyPropagate(void);
extern RP rmaTestCg(RP pcode, U vt, UB* value);
extern void hsUpdateModpathTimeStamp(UB* pmps);
extern void doMpd32One(UB* pmps);
extern void doMpdCommon(MPS* pmps);
extern TimeStamp GET_DIFF_DELAY_FUNC(TimeStamp ts);
extern void SchedSemiLerMP(UB* ppulse, U partId);
extern void SchedSemiLerMP_front(UB* ppulse, U partId);
extern EBLK *peblkFutQ1Head;
extern EBLK *peblkFutQ1Tail;
extern void vcs_simpSetEBlkEvtID(EBLK *peblk);
extern void scheduleuna(UB *e, U t);
extern void scheduleuna_mp(EBLK *e, unsigned t);
extern void schedule(UB *e, U t);
extern void sched_hsopt(struct dummyq_struct * pQ, EBLK *e, U t);
extern void sched_millenium(struct dummyq_struct * pQ, void *e, U thigh, U t);
extern void schedule_1(EBLK *e);
extern void sched0(UB *e);
extern void sched0lq(EBLK *e);
extern void sched0lqnc(EBLK *e);
extern void sched0una(UB *e);
extern void sched0una_th(struct dummyq_struct *pq, UB *e);
extern void hsopt_sched0u_th(struct dummyq_struct *pq, UB *e);
extern void scheduleuna_mp_th(struct dummyq_struct *pq, EBLK *e, unsigned t);
extern void scheduleuna_mp_front_th(struct dummyq_struct *pq, EBLK *e, unsigned t);
extern void schedal(UB *e);
extern void sched0_th(struct dummyq_struct * pQ, EBLK *e);
extern void sched0u(UB *e);
extern void sched0u_th(struct dummyq_struct *pq, UB *e);
extern void sched0_hsim_front_th(struct dummyq_struct * pQ, UB *e);
extern void sched0_hsim_frontlq_th(struct dummyq_struct * pQ, UB *e);
extern void sched0lq_th(struct dummyq_struct * pQ, UB *e);
extern void schedal_th(struct dummyq_struct * pQ, UB *e);
extern void scheduleuna_th(struct dummyq_struct * pQ, void *e, U t);
extern void sched64cuna_th(struct dummyq_struct * pQ, void *e, U h, U l);
extern void schedule_th(struct dummyq_struct * pQ, UB *e, U t);
extern void schedule_1_th(struct dummyq_struct * pQ, EBLK *peblk);
extern void schedule_1_front_th(struct dummyq_struct * pQ, EBLK *peblk);
extern void SetupLER_th(struct dummyq_struct * pQ, EBLK *e);
extern void FsdbReportClkGlitch(UB*,U);
extern void AddToClkGLitchArray(EBLK*);
extern void SchedSemiLer_th(struct dummyq_struct * pQ, EBLK *e);
extern void SchedSemiLerTXP_th(struct dummyq_struct * pQ, EBLK *e);
extern void SchedSemiLerTXPFreeVar_th(struct dummyq_struct * pQ, EBLK *e);
extern U getVcdFlags(UB *ip);
extern void VVrpNonEventNonRegdScalarForHsimOpt(void* ip, U nIndex);
extern void VVrpNonEventNonRegdScalarForHsimOpt2(void* ip, U nIndex);
extern void SchedSemiLerTBReactiveRegion(struct eblk* peblk);
extern void SchedSemiLerTBReactiveRegion_th(struct eblk* peblk, U partId);
extern void SchedSemiLerTr(UB* peblk, U partId);
extern void SchedSemiLerNBA(UB* peblk, U partId);
extern void NBA_Semiler(void *ip, void *pNBS);
extern void sched0sd_hsim(UB* peblk);
extern void vcs_sched0sd_hsim_udpclk(UB* peblk);
extern void vcs_sched0sd_hsim_udpclkopt(UB* peblk);
extern void sched0sd_hsim_PRL(UB* peblk);
extern void sched0lq_parallel_clk(EBLK* peblk);
extern void sched0lq_de(U* pe, void* ip, void* prout);
extern void sched0lq_hsopt_nonfgp_de(U* pe, void* ip, void* prout);
extern void sched0lq_hsopt_parallel_de(U* pe, void* ip, void* prout);
extern void sched0_de_inl(void* ip, void* prout);
extern void rmaPostDynamicSchedDeltaGate(UB* pcode);
extern U    isRtlClockScheduled(EBLK* peblk);
extern void doFgpRaceCheck(UB* pcode, UB* p, U flag);
extern void doSanityLvlCheck();
extern void sched0lq_dbsed_ova_jaguar(EBLK* peblk);
extern void sched0lq_nondbsed_ova_jaguar(EBLK* peblk);
extern void sched0lq_parallel_ova(EBLK* peblk);
extern void sched0lq_parallel_ova_precheck(EBLK* peblk);
extern void rmaDlpEvalSeqPrim(UB* peblk, UB val, UB preval);
extern void appendNtcEvent(UB* phdr, scalar s, U schedDelta);
extern void appendTransEventS(RmaTransEventHdr* phdr, scalar s, U schedDelta);
extern void schedRetainHsim(MPS* pMPS, scalar sv, scalar pv);
extern void updateRetainHsim(MPS* pMPS,scalar sv, scalar pv);
extern void hsimCountXEdges(void* record,  scalar s);
extern void hsimRegisterEdge(void* sm,  scalar s);
extern EBLK* pvcsGetLastEventEblk(U thid);
extern void insertTransEventIC(RmaTransEventHdr* phdr, scalar s, scalar pv, scalar resval, U schedDelta, int re, UB* predd, U fpdd);
extern void insertNtcEventRF(RmaTransEventHdr* phdr, scalar s, scalar pv, scalar resval, U schedDelta, U* delays);
extern U doTimingViolation(RmaTimeStamp ts,RP* pdata, U fskew, U limit, U floaded, U fcondopt, RmaTimeStamp tsNochange);
extern void sched_gate_hsim(EBLK* peblk, unsigned t, RP* offset, U gd_info, U encodeInPcode, void* propValue);
extern void sched_gate_hsim_front(EBLK* peblk, unsigned t, RP* offset, U gd_info, U encodeInPcode, void* propValue);
extern int getCurSchedRegion();
extern FP getRoutPtr(RP, U);
extern U rmaChangeCheckAndUpdateE(scalar* pvalDst, scalar* pvalSrc, U cbits);
extern void rmaUpdateE(scalar* pvalDst, scalar* pvalSrc, U cbits);
extern U rmaChangeCheckAndUpdateEFromW(scalar* pvalDst, vec32* pvalSrc, U cbits);
extern void rmaLhsPartSelUpdateE(scalar* pvalDst, scalar* pvalSrc, U index, U width);
extern void rmaUpdateWithForceSelectorE(scalar* pvalDst, scalar* pvalSrc, U cbits, U* pforceSelector);
extern void rmaUpdateWFromE(vec32* pvalDst, scalar* pvalSrc, U cbits);
extern void rmaUpdateWUFromE(U* pv, scalar* pval, U cbits);
extern U rmaLhsPartSelWithChangeCheckE(scalar* pvalDst, scalar* pvalSrc, U index, U width);
extern void rmaLhsPartSelWFromE(vec32* pvalDst, scalar* pvalSrc, U index,U width);
extern U rmaChangeCheckAndUpdateW(vec32* pvalDst, vec32* pvalSrc, U cbits);
extern void rmaUpdateW(vec32* pvalDst, vec32* pvalSrc, U cbits);
extern void rmaUpdateEFromW(scalar* pvalDst, vec32* pvalSrc, U cbits);
extern void *VCSCalloc(size_t size, size_t count);
extern void *VCSMalloc(size_t size);
extern void VCSFree(void *ptr);
extern U rmaLhsPartSelWithChangeCheckW(vec32* pvalDst, vec32* pvalSrc, U index,U width);
extern void rmaLhsPartSelEFromW(scalar* pvalDst, vec32* pvalSrc, U index,U width);
extern U rmaLhsPartSelWithChangeCheckEFromW(scalar* pvalDst, vec32* pvalSrc, U index,U width);
extern void rmaLhsPartSelUpdateW(vec32* pvalDst, vec32* pvalSrc, U index, U width);
extern void rmaEvalWunionW(vec32* dst, vec32* src, U cbits, U count);
extern void rmaEvalWorW(vec32* dst, vec32* src, U cbits, U count);
extern void rmaEvalWandW(vec32* dst, vec32* src, U cbits, U count);
extern void rmaEvalUnionE(scalar* dst, scalar* src, U cbits, U count, RP ptable);
typedef U RmaCgFunctionType;
extern RmaIbfPcode* rmaEvalPartSelectsW(vec32* pvec32, U startIndex, U onWidth, U offWidth, U count, RmaIbfPcode* pibfPcode, U fnonRootForce, UB* pevcdStatus);
extern RmaIbfPcode* rmaEvalPartSelectsWLe32(vec32* pvec32, U startIndex, U onWidth, U offWidth, U count, RmaIbfPcode* pibfPcode, U fnonRootForce, UB* pevcdStatus);
extern RmaIbfPcode* rmaEvalPartSelectsWToE(vec32* pvec32, U startIndex, U onWidth, U offWidth, U count, RmaIbfPcode* pibfPcode, U fnonRootForce);
extern RmaIbfPcode* rmaEvalPartSelectsEToE(scalar* pv, U startIndex, U onWidth, U offWidth, U count, RmaIbfPcode* pibfPcode, U fnonRootForce, UB* pevcdStatus);
extern RmaIbfPcode* rmaEvalPartSelectsEToW(scalar* pv, U startIndex, U onWidth, U offWidth, U count, RmaIbfPcode* pibfPcode, U fnonRootForce);
extern U rmaEvalBitPosEdgeW(vec32* pvalCurr, vec32* pvalPrev, U cbits, U* pedges);
extern U rmaEvalBitNegEdgeW(vec32* pvalCurr, vec32* pvalPrev, U cbits, U* pedges);
extern U rmaEvalBitChangeW(vec32* pvalCurr, vec32* pvalPrev, U cbits, U* pedges);
extern U VcsForceVecVCg(UB* pcode, UB* pval, UB* pvDst, UB* pvCur, U fullcbits, U ibeginSrc, U ibeginDst, U width, U/*RmaValueConvType*/ convtype, U/*RmaForceType*/ frcType, UB* prhs, UB* prhsDst, U frhs, U* pforcedbits, U fisRoot);
extern U VcsReleaseVecVCg(UB* pcode, UB* pvDst, U fullcbits, U ibeginDst, U width, UB* prhsDst, U frhs, U* pforcedbits, U fisRoot);
extern U VcsForceVecWCg(UB* pcode, UB* pval, UB* pvDst, UB* pvCur, U fullcbits, U ibeginSrc, U ibeginDst, U width, U/*RmaValueConvType*/ convtype, U /*RmaForceType*/ frcType, UB* prhs, UB* prhsDst, U frhs, U* pforcedbits, U fisRoot);
extern U VcsReleaseVecWCg(UB* pcode, UB* pvDst, U fullcbits, U ibeginDst, U width, UB* prhsDst, U frhs, U* pforcedbits, U fisRoot);
extern U VcsForceVecECg(UB* pcode, UB* pval, UB* pvDst, UB* pvCur, U fullcbits, U ibeginSrc, U ibeginDst, U width, U /*RmaValueConvType*/ convtype, U /*RmaForceType*/ frcType,UB* prhs, UB* prhsDst, U frhs, U* pforcedbits, U fisRoot);
extern U VcsForceVecACg(UB* pcode, UB* pval, UB* pvDst, UB* pvCur, U fullcbits, U ibeginSrc, U ibeginDst, U width, U /*RmaValueConvType*/ convtype, U /*RmaForceType*/ frcType,UB* prhs, UB* prhsDst, U frhs, U* pforcedbits, U fisRoot);
extern U VcsReleaseVecCg(UB* pcode, UB* pvDst, U ibeginDst, U width, U /*RmaValueType*/ type,U fisRoot, UB* prhsDst, U frhs, U* pforcedbits);
extern U VcsDriveBitsAndDoChangeCheckV(vec32* pvSel, vec32* pvCur, U fullcbits, U* pforcedbits, U isRoot);
extern U VcsDriveBitsAndDoChangeCheckW(vec32* pvSel, vec32* pvCur, U fullcbits, U* pforcedbits, U isRoot);
extern U VcsDriveBitsAndDoChangeCheckE(scalar* pvSel, scalar* pvCur, U fullcbits, U* pforcedbits, U isRoot);
extern void cgvecDebug_Eblk(UB* pcode);
extern U rmaCmpW(vec32* pvalDst, vec32* pvalSrc, U index, U width);
extern void copyVec32ArrMask(vec32* pv1, vec32* pv2, U len, U* mask);
extern void* memcpy(void*, const void*, size_t);
extern int memcmp(const void*, const void*, size_t);
extern void propagateScanOptPathVal(EBLK *peblk);
extern UB* rmaProcessRtvsSwitches(UB* pcode, scalar val);
extern UB* rmaProcessScanSwitches(UB* pcode, scalar val);
extern UB* rmaProcessScanSwitchesV(UB* pcode, vec32 *pval);
extern UB* rmaProcessScanoptDump(UB* pcode, scalar val);
extern UB* rmaProcessScanoptDumpV(UB* pcode, vec32 *pval);
extern UB* rmaProcessScanChainOptSeqPrims(UB* pcode, scalar val);
extern void schedResetRecoveryDbs(U cedges, EBLK* peblkFirst);
extern UB* rmaEvalUnaryOpV(UB* pcode, vec32* pval);
extern UB* rmaEvalBinaryOpV(UB* pcode, vec32* pval);
extern UB* rmaEvalBinaryOpVOneFanoutCount(UB* pcode, vec32* pval);
extern UB* rmaEvalBinaryOpVLargeFanoutCount(UB* pcode, vec32* pval);
extern UB* rmaEvalAndOpVOneFanoutCount(UB* pcode, vec32* value);
extern UB* rmaEvalAndOpVLargeFanoutCount(UB* pcode, vec32* value);
extern UB* rmaEvalAndOpV(UB* pcode, vec32* value);
extern UB* rmaEvalOrOpVOneFanoutCount(UB* pcode, vec32* value);
extern UB* rmaEvalOrOpVLargeFanoutCount(UB* pcode, vec32* value);
extern UB* rmaEvalOrOpV(UB* pcode, vec32* value);
extern UB* rmaEvalTernaryOpV(UB* pcode, vec32* pval);
extern UB* rmaEvalUnaryOpW(UB* pcode, vec32* pval);
extern UB* rmaEvalBinaryOpW(UB* pcode, vec32* pval);
extern UB* rmaEvalTernaryOpW(UB* pcode, vec32* pval);
extern UB* rmaEvalUnaryOpE(UB* pcode, scalar* pv);
extern UB* rmaEvalBinaryOpE(UB* pcode, scalar* pv);
extern UB* rmaEvalTernaryOpE(UB* pcode, scalar* pv);
extern UB* rmaEvalTernaryOpS(UB* pcode, scalar val);
extern scalar rmaGetScalarFromWCg(vec32* pval, U index);
extern void rmaSetScalarInWCg(vec32* pval, U index, scalar s);
extern void rmaSetWInW(vec32* dst, vec32* src, U index, U indexSrc, U width);
extern void setHsimFunc(void* ip, U offset);
extern void vcs_updateHsimFunc(void* ip, U offset);
extern void unsetHsimFunc(void* ip);
extern U fProfEvtProp;
extern UB* getEvcdStatusByFlagsE(scalar* pscalar, UB* pevcdTBDriverFlags, U cdrivers, UB* table, U cbits);
extern UB* getEvcdStatusByFlagsV(vec32* pvec32, UB* pevcdTBDriverFlags, U cdrivers, UB* table, U cbits);
extern UB* getEvcdStatusByFlagsW(vec32* pvec32, UB* pevcdTBDriverFlags, U cdrivers, UB* table, U cbits);
extern UB* getEvcdStatusByFlagsS(scalar* pscalar, UB* pevcdTBDriverFlags, U cdrivers, UB* table);
extern UB* getSingleDrvEvcdStatusS(UB value, U fTBDriver);
extern UB* getSingleDrvEvcdStatusE(scalar* pscalars, U fTBDriver, U cbits);
extern UB* getSingleDrvEvcdStatusV(scalar* pscalars, U fTBDriver, U cbits);
extern UB* getSingleDrvEvcdStatusW(scalar* pscalars, U fTBDriver, U cbits);
extern UB* getEvcdStatusByDrvEvcdStatus(UB* pdrvevcdStatus, U cdrivers, UB* table, U cbits);
extern void evcdCallback(UP pcode, U cbits);
extern UB* getSavedEvcdStatus(void);
extern void saveEvcdStatus(UB*);
extern void mhdlMarkExport(void*, U);
extern void levelInsertQueue(int);
extern void levelInsertQueueGL(int);
extern void VcsRciRtl(RP pcode);
extern void VcsRciSCompiledLoad(scalar* pcode, scalar ibnVal);
extern U fLoopDetectMode;
extern int gFLoopDectCodeEna;
extern U fLoopReportRT;
extern void rtSched0LoopDectDumpProcess(void* e, void* rtn, void* PQ);
extern void pushHsimRtnCtxt(void* pcode);
extern void popHsimRtnCtxt();
extern EBLK* loopReportInlinedSched0Wrapper(EBLK *peblk);
extern void loopReportSched0Wrapper(EBLK *peblk, unsigned int sfType, unsigned int fTH, struct dummyq_struct* pq);
extern void loopReportSchedSemiLerWrapper(EBLK *peblk, int sfType);
extern void CallGraphPushNodeAndAddToGraph(UP flatNode, UP instNum, U dummy);
extern void setChildClockWriteFuncAndPcode( RP pcode, RP fn);
extern void CallGraphPopNode(void);
extern RP elabGetIpTpl(U in);
extern U rmaEvalBitBothEdgeW(vec32* pvalCurr, vec32* pvalPrev, U cbits, U* pedges);
extern U rmaEvalBitEdgeQ1W(vec32* pvalCurr, vec32* pvalPrev, U cbits, U* pedges);
extern U rmaEvalBitEdgeQXW(vec32* pvalCurr, vec32* pvalPrev, U cbits, U* pedges);
extern U rmaEvalBitEdgeQ0W(vec32* pvalCurr, vec32* pvalPrev, U cbits, U* pedges);
extern U rmaEval01EdgeW(vec32* pvalCurr, vec32* pvalPrev, U cbits, U* pedges);
extern U rmaEval0XEdgeW(vec32* pvalCurr, vec32* pvalPrev, U cbits, U* pedges);
extern U rmaEval10EdgeW(vec32* pvalCurr, vec32* pvalPrev, U cbits, U* pedges);
extern U rmaEval1XEdgeW(vec32* pvalCurr, vec32* pvalPrev, U cbits, U* pedges);
extern U rmaEvalX1EdgeW(vec32* pvalCurr, vec32* pvalPrev, U cbits, U* pedges);
extern U rmaEvalX0EdgeW(vec32* pvalCurr, vec32* pvalPrev, U cbits, U* pedges);
extern U rmaEvalBitPosEdgeE(scalar* pvalCurr, scalar* pvalPrev, U cbits, U* pedges);
extern U rmaEvalBitNegEdgeE(scalar* pvalCurr, scalar* pvalPrev, U cbits, U* pedges);
extern U rmaEvalBitBothEdgeE(scalar* pvalCurr, scalar* pvalPrev, U cbits, U* pedges);
extern U rmaEvalBitEdgeQ1E(scalar* pvalCurr, scalar* pvalPrev, U cbits, U* pedges);
extern U rmaEvalBitEdgeQ0E(scalar* pvalCurr, scalar* pvalPrev, U cbits, U* pedges);
extern U rmaEvalBitChangeE(scalar* pvalCurr, scalar* pvalPrev, U cbits, U* pedges);
extern void rmaScheduleNbaGate(RP pcode, scalar val);
extern void rmaEvalRtlEdgeLoads(RmaRtlEdgeBlockHdr *phdr, US clkEdge, scalar clkVal, scalar prevClkVal, scalar val4,   scalar prevval4, scalar master4val);
extern void rmaEvalRtlEdgeLoadsJaguar(RmaRtlEdgeBlockHdr *phdr, US clkEdge, scalar clkVal, scalar prevClkVal, scalar val4,   scalar prevval4, scalar master4val);
extern void rmaEvaluateDynamicGateLoadsCg(RP p, scalar s);
extern void rmaEvaluateFusedWithDynamicGateLoadsCg(RP p, scalar s);
extern void rmaScheduleGatedClockEdgeLoadNew(UB* p, US* ea, U flags, UB* plist, UB* pprevlist, scalar v, scalar pv);
extern void rmaScheduleGatedClockEdgeLoad(UB* p, US* ea, U flags, UB* plist, UB* pprevlist, scalar v);
extern void vcsCheckIfClkValValid(U val, U index);
extern void rmaRemoveNonEdgeLoads(UB* pcode);
extern void rmaRecordEvents(HsimNodeRecord *pnr);
extern void handlePCBs(UB* p, U i);
extern void markMasterClkOvaLists(U fdbs, RP p);
extern void rmaChildClockPropAfterWrite(UB* p);
extern void rmaSchedChildClockPropAfterWrite(UB* p, UB* pmasterList, UB val);
extern void rmaChangeListForMovedGates(UB clkVal, UB f10Edge, UB* subMasterVal, UB* plist, RP* p, U count);
extern void rmaEvalSeqPrimLoadsByteArray(UB* pcode, UB val, UB prevval4);
extern void rmaEvalSeqPrimLoadsByteArrayX(UB* pcode, UB val, UB prevval4);
extern void vcsRmaEvalSeqPrimLoadsByteArraySCT(UB* pcode, UB val, UB prevval4, U c);
extern void rmaEvalSimonCellLoadsByteArray(UB* pcode, UB val, UB prevval4);
extern void rmaEvalSimonCellLoadsByteArrayX(UB* pcode, UB val, UB prevval4);
extern void vcsAbortForBadEBlk(void);
extern scalar edgeChangeLookUp[4][4];
extern void Wsvvar_sched_virt_intf_eval(RP* ptr, char val);
extern void vcs_hwcosim_drive_dut_scalar(U id, char val);
extern void vcs_hwcosim_drive_dut_vector_4state(U id, vec32* val);
extern U vcs_rmaGetClkValForSeqUdpLayoutOnClkOpt(UB* poutput);
extern U vcs_rmaGetClkValForSeqUdpLayoutOnClk(UB* poutput);
extern U vcs_rmaGetClkValForSeqUdp(UB* poutput);
extern void vcs_rmaGetWFForSeqUdp(UB* phead);
extern U add_new_event(void* wfclock, unsigned char s);
extern U has_periodicity_started(void *wfclock);
extern void fire_scalarclock_start_cbk(void* wfclock, void* pdata);
extern void fire_scalarclock_end_cbk(void* wfclock);
extern U is_fsdb_periodic(void *wfclock);
extern void patchWfTs(RP _ppwf);
extern void patchWfWp(RP _ppwf);
extern void patchWfS(RP _ppwf);
extern void patchWfH(RP _ppwf);
extern U is_connected(void* wfclock, U idx);
extern U is_delay_connected(void* wfclock, U idx, scalar *out_prev_val);
extern void set_auto_sched_for_dbs_loads(void* wfclock, scalar edge);
extern U Fsim_handle_wreal_faults(void);
extern U is_event_record_needed(void *waveform);
extern scalar get_value_for_wf(void* ip);
extern U rmaIsS2State(scalar s);
extern U rmaIsV2State(vec32* pval, U cbits);
extern U rmaIsW2State(vec32* pval, U cbits);
extern U rmaIsE2State(scalar* pval, U cbits);
extern void rmaUpdateRecordFor2State(HsimNodeRecord* record, U f2state);
typedef void (*FuncPtr)();
static inline U asm_bsf (U in)
{
#if defined(linux)
    U out;
#if !defined(__aarch64__)
    asm ("movl %1, %%eax; bsf %%eax, %%eax; movl %%eax, %0;"
        :"=r"(out)
        :"r"(in)
        :"%eax"
        );
#else
    out = ffs(in) - 1;
#endif
    return out;
#else
    return 0;
#endif
}


void  rmaPropagate0_p_simv_wrapper_daidir (UB  * pcode, scalar  val);
void  rmaPropagate0_simv_wrapper_daidir (UB  * pcode, scalar  val);
void  rmaPropagate0_f_simv_wrapper_daidir (UB  * pcode, scalar  val, U  I612, scalar  * I1504, U  did);
void  rmaPropagate0_r_simv_wrapper_daidir (UB  * pcode);
void  rmaPropagate0_wn_simv_wrapper_daidir (UB  * pcode, scalar  val);
void  rmaPropagate3_p_simv_wrapper_daidir (UB  * pcode, scalar  val);
void  rmaPropagate3_simv_wrapper_daidir (UB  * pcode, scalar  val);
void  rmaPropagate3_wn_simv_wrapper_daidir (UB  * pcode, scalar  val);
void  rmaPropagate11_p_simv_wrapper_daidir (UB  * pcode, scalar  val);
void  rmaPropagate11_simv_wrapper_daidir (UB  * pcode, scalar  val);
void  rmaPropagate11_wn_simv_wrapper_daidir (UB  * pcode, scalar  val);
void  rmaPropagate13_p_simv_wrapper_daidir (UB  * pcode, scalar  val);
void  rmaPropagate13_simv_wrapper_daidir (UB  * pcode, scalar  val);
void  rmaPropagate13_f_simv_wrapper_daidir (UB  * pcode, scalar  val, U  I612, scalar  * I1504, U  did);
void  rmaPropagate13_r_simv_wrapper_daidir (UB  * pcode);
void  rmaPropagate13_wn_simv_wrapper_daidir (UB  * pcode, scalar  val);
void  rmaPropagate14_p_simv_wrapper_daidir (UB  * pcode, scalar  val);
void  rmaPropagate14_simv_wrapper_daidir (UB  * pcode, scalar  val);
void  rmaPropagate14_f_simv_wrapper_daidir (UB  * pcode, scalar  val, U  I612, scalar  * I1504, U  did);
void  rmaPropagate14_r_simv_wrapper_daidir (UB  * pcode);
void  rmaPropagate14_wn_simv_wrapper_daidir (UB  * pcode, scalar  val);
void  rmaPropagate15_p_simv_wrapper_daidir (UB  * pcode, scalar  val);
void  rmaPropagate15_simv_wrapper_daidir (UB  * pcode, scalar  val);
void  rmaPropagate15_f_simv_wrapper_daidir (UB  * pcode, scalar  val, U  I612, scalar  * I1504, U  did);
void  rmaPropagate15_r_simv_wrapper_daidir (UB  * pcode);
void  rmaPropagate15_wn_simv_wrapper_daidir (UB  * pcode, scalar  val);
void  rmaPropagate17_p_simv_wrapper_daidir (UB  * pcode, scalar  val);
void  rmaPropagate17_simv_wrapper_daidir (UB  * pcode, scalar  val);
void  rmaPropagate17_f_simv_wrapper_daidir (UB  * pcode, scalar  val, U  I612, scalar  * I1504, U  did);
void  rmaPropagate17_r_simv_wrapper_daidir (UB  * pcode);
void  rmaPropagate17_wn_simv_wrapper_daidir (UB  * pcode, scalar  val);
void  rmaPropagate20_p_simv_wrapper_daidir (UB  * pcode, scalar  val);
void  rmaPropagate20_simv_wrapper_daidir (UB  * pcode, scalar  val);
void  rmaPropagate20_f_simv_wrapper_daidir (UB  * pcode, scalar  val, U  I612, scalar  * I1504, U  did);
void  rmaPropagate20_r_simv_wrapper_daidir (UB  * pcode);
void  rmaPropagate20_wn_simv_wrapper_daidir (UB  * pcode, scalar  val);
void  rmaPropagate21_p_simv_wrapper_daidir (UB  * pcode, scalar  val);
void  rmaPropagate21_simv_wrapper_daidir (UB  * pcode, scalar  val);
void  rmaPropagate21_f_simv_wrapper_daidir (UB  * pcode, scalar  val, U  I612, scalar  * I1504, U  did);
void  rmaPropagate21_r_simv_wrapper_daidir (UB  * pcode);
void  rmaPropagate21_wn_simv_wrapper_daidir (UB  * pcode, scalar  val);
void  rmaPropagate24_p_simv_wrapper_daidir (UB  * pcode, scalar  val);
void  rmaPropagate24_simv_wrapper_daidir (UB  * pcode, scalar  val);
void  rmaPropagate24_f_simv_wrapper_daidir (UB  * pcode, scalar  val, U  I612, scalar  * I1504, U  did);
void  rmaPropagate24_r_simv_wrapper_daidir (UB  * pcode);
void  rmaPropagate24_wn_simv_wrapper_daidir (UB  * pcode, scalar  val);
void  rmaPropagate26_p_simv_wrapper_daidir (UB  * pcode, scalar  val);
void  rmaPropagate26_simv_wrapper_daidir (UB  * pcode, scalar  val);
void  rmaPropagate26_f_simv_wrapper_daidir (UB  * pcode, scalar  val, U  I612, scalar  * I1504, U  did);
void  rmaPropagate26_r_simv_wrapper_daidir (UB  * pcode);
void  rmaPropagate26_wn_simv_wrapper_daidir (UB  * pcode, scalar  val);
void  rmaPropagate27_p_simv_wrapper_daidir (UB  * pcode, scalar  val);
void  rmaPropagate27_simv_wrapper_daidir (UB  * pcode, scalar  val);
void  rmaPropagate27_f_simv_wrapper_daidir (UB  * pcode, scalar  val, U  I612, scalar  * I1504, U  did);
void  rmaPropagate27_r_simv_wrapper_daidir (UB  * pcode);
void  rmaPropagate27_t0_simv_wrapper_daidir (UB  * pcode, UB  val);
void  rmaPropagate27_wn_simv_wrapper_daidir (UB  * pcode, scalar  val);
void  rmaPropagate28_p_simv_wrapper_daidir (UB  * pcode, scalar  val);
void  rmaPropagate28_simv_wrapper_daidir (UB  * pcode, scalar  val);
void  rmaPropagate28_f_simv_wrapper_daidir (UB  * pcode, scalar  val, U  I612, scalar  * I1504, U  did);
void  rmaPropagate28_r_simv_wrapper_daidir (UB  * pcode);
void  rmaPropagate28_wn_simv_wrapper_daidir (UB  * pcode, scalar  val);
void  rmaPropagate29_p_simv_wrapper_daidir (UB  * pcode, scalar  val);
void  rmaPropagate29_simv_wrapper_daidir (UB  * pcode, scalar  val);
void  rmaPropagate29_f_simv_wrapper_daidir (UB  * pcode, scalar  val, U  I612, scalar  * I1504, U  did);
void  rmaPropagate29_r_simv_wrapper_daidir (UB  * pcode);
void  rmaPropagate29_wn_simv_wrapper_daidir (UB  * pcode, scalar  val);
void  rmaPropagate31_p_simv_wrapper_daidir (UB  * pcode, scalar  val);
void  rmaPropagate31_simv_wrapper_daidir (UB  * pcode, scalar  val);
void  rmaPropagate31_f_simv_wrapper_daidir (UB  * pcode, scalar  val, U  I612, scalar  * I1504, U  did);
void  rmaPropagate31_r_simv_wrapper_daidir (UB  * pcode);
void  rmaPropagate31_wn_simv_wrapper_daidir (UB  * pcode, scalar  val);
void  rmaPropagate32_simv_wrapper_daidir (UB  * pcode, scalar  val);
void  schedNewEvent (struct dummyq_struct * I1482, EBLK  * I1477, U  I619);

#ifdef __cplusplus
  }
#endif
#endif /*__DO_RMAHDR_*/

