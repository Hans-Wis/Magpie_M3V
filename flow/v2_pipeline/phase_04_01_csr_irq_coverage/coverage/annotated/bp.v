...
%000001     output            bp_predict_taken,
%000001     output     [31:0] bp_predict_target,
        
            // ---- Write port (synchronous, BP update from MEM stage) ----
%000002     input             upd_valid,
~000200     input      [31:0] upd_pc,
%000001     input             upd_taken,
...
%000001     reg                 lru      [0:N_SETS-1];           // 0 = way0 是 LRU；1 = way1 是 LRU
...
%000001     wire rd_hit0 = valid0[rd_idx] && (tag0[rd_idx] == rd_tag);
...
%000001     wire predict_from_way0 = rd_hit0 && counter0[rd_idx][1];
...
%000001     wire wr_hit0 = valid0[wr_idx] && (tag0[wr_idx] == wr_tag);
...
~001320                   wr_hit0 ? 1'b0 :
 001320                             lru[wr_idx];
%000001     wire wr_hit = wr_hit0 | wr_hit1;
...
~000284             if (wr_way == 1'b1) begin
%000000                 valid1[wr_idx]   <= 1'b1;
%000000                 tag1[wr_idx]     <= wr_tag;
%000000                 target1[wr_idx]  <= upd_target;
%000000                 counter1[wr_idx] <= cnt_next;
~000284             end else begin
~000284                 valid0[wr_idx]   <= 1'b1;
~000284                 tag0[wr_idx]     <= wr_tag;
~000284                 target0[wr_idx]  <= upd_target;
~000284                 counter0[wr_idx] <= cnt_next;
                    end
~000284             lru[wr_idx] <= lru_next;
                end
            end
