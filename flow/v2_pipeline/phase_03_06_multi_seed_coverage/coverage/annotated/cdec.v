//      // verilator_coverage annotation
        ...
        module cdec (
~000131     input      [15:0] cinstr,
~000020     output reg [31:0] expanded,
 000025     output reg        illegal
        );
        
            // -------------------------------------------------------------------------
            // 提取常用 field
            // -------------------------------------------------------------------------
 000021     wire [1:0] op       = cinstr[1:0];        // quadrant
 000077     wire [2:0] funct3   = cinstr[15:13];      // 主功能
 000131     wire [4:0] rd_rs1_5 = cinstr[11:7];       // 5-bit (Q2 / 部分 Q1)
~000079     wire [4:0] rs2_5    = cinstr[6:2];        // 5-bit (Q2)
 000131     wire [2:0] rd_rs1_3 = cinstr[9:7];        // 3-bit "rs1'/rd'" (Q1)
 000058     wire [2:0] rs2_3    = cinstr[4:2];        // 3-bit "rs2'/rd'" (Q0/Q1)
 000131     wire [2:0] rs1_q0_3 = cinstr[9:7];        // 3-bit (Q0)
 000058     wire [2:0] rd_q0_3  = cinstr[4:2];        // 3-bit (Q0 c.lw target)
        
            // 3-bit → 5-bit (映射 x8-x15)
~000131     wire [4:0] rd_rs1_p = {2'b01, rd_rs1_3};
~000058     wire [4:0] rs2_p    = {2'b01, rs2_3};
...
~000131     wire [11:0] imm_addi4spn = {2'b0, cinstr[10:7], cinstr[12:11], cinstr[5], cinstr[6], 2'b0};
        
            // C.LW / C.SW: imm[5:3|2|6] from cinstr[12:10,6,5] (zext)
~000079     wire [11:0] imm_lwsw = {5'b0, cinstr[5], cinstr[12:10], cinstr[6], 2'b0};
        
            // C.ADDI / C.LI / C.ANDI: imm[5|4:0] sign-ext
~000079     wire [11:0] imm_addi = {{7{cinstr[12]}}, cinstr[6:2]};
        
            // C.LUI: imm[17|16:12] sign-ext (Notice: LUI imm 是 32-bit imm[31:12])
~000079     wire [19:0] imm_lui  = {{15{cinstr[12]}}, cinstr[6:2]};
        
            // C.ADDI16SP: imm[9|4|6|8:7|5] sign-ext (10-bit imm, 16-byte align)
~000079     wire [11:0] imm_addi16sp = {{3{cinstr[12]}}, cinstr[4:3], cinstr[5], cinstr[2], cinstr[6], 4'b0};
        
            // C.SRLI / C.SRAI / C.SLLI shamt: cinstr[12,6:2] (6-bit but RV32 只用低 5-bit)
~000079     wire [4:0] shamt = cinstr[6:2];
        
            // C.J / C.JAL: imm[11|4|9:8|10|6|7|3:1|5] sign-ext (11-bit aligned to 2-byte) → 21-bit
~000131     wire [20:0] imm_cj = {{10{cinstr[12]}}, cinstr[8], cinstr[10:9], cinstr[6], cinstr[7],
                                  cinstr[2], cinstr[11], cinstr[5:3], 1'b0};
        
            // C.BEQZ / C.BNEZ: imm[8|4:3|7:6|2:1|5] sign-ext (9-bit aligned to 2-byte) → 13-bit
~000079     wire [12:0] imm_cb = {{5{cinstr[12]}}, cinstr[6:5], cinstr[2], cinstr[11:10], cinstr[4:3], 1'b0};
        
            // C.LWSP: imm[5|4:2|7:6] from cinstr[12,6:4,3:2] zext
~000079     wire [11:0] imm_lwsp = {4'b0, cinstr[3:2], cinstr[12], cinstr[6:4], 2'b0};
        
            // C.SWSP: imm[5:2|7:6] from cinstr[12:9,8:7] zext
~000131     wire [11:0] imm_swsp = {4'b0, cinstr[8:7], cinstr[12:9], 2'b0};
        
 001144     always @* begin
 001144         expanded = 32'h0;
 001144         illegal  = 1'b0;
...
%000001             3'b010: begin
                        // C.LW: lw rd', imm(rs1')
%000001                 expanded = {imm_lwsw, rd_rs1_p, 3'b010, rs2_p, `OPC_LOAD};
                    end
%000000             3'b110: begin
                        // C.SW: sw rs2', imm(rs1')
%000000                 expanded = {imm_lwsw[11:5], rs2_p, rd_rs1_p, 3'b010, imm_lwsw[4:0], `OPC_STORE};
                    end
...
%000000                 expanded = {imm_cj[20], imm_cj[10:1], imm_cj[11], imm_cj[19:12], 5'd1, `OPC_JAL};
 ...
 000016                 expanded = {imm_addi, 5'd0, 3'b000, rd_rs1_5, `OPC_OP_IMM};
                    end
%000000             3'b011: begin
%000000                 if (rd_rs1_5 == 5'd2) begin
...
%000000                     if (cinstr[12] == 1'b0 && cinstr[6:2] == 5'b0) illegal = 1'b1;
%000000                     else expanded = {imm_lui, rd_rs1_5, `OPC_LUI};
                        end
                    end
%000001             3'b100: begin
...
%000000             3'b101: begin
                        // C.J: jal x0, imm
%000000                 expanded = {imm_cj[20], imm_cj[10:1], imm_cj[11], imm_cj[19:12], 5'd0, `OPC_JAL};
                    end
%000000             3'b110: begin
...
%000001                 expanded = {imm_cb[12], imm_cb[10:5], 5'd0, rd_rs1_p, 3'b001,
%000001                             imm_cb[4:1], imm_cb[11], `OPC_BRANCH};
                    end
                endcase
                ...
                // Quadrant 2 (op=10)
                // =====================================================================
 000049         2'b10: case (funct3)
%000001             3'b000: begin
                        // C.SLLI: slli rd, rd, shamt
%000001                 if (cinstr[12]) illegal = 1'b1;     // RV32 shamt>=32 reserved -> trap
...
~000043                 if (cinstr[12] == 1'b0) begin
...
%000005                         expanded = {7'b0, rs2_5, 5'd0, 3'b000, rd_rs1_5, `OPC_OP};
                            end
 000043                 end else begin    // cinstr[12]==1
~000043                     if (rs2_5 == 5'd0) begin
~000043                         if (rd_rs1_5 == 5'd0) begin
                                    // C.EBREAK
 000043                             expanded = 32'h0010_0073;
%000000                         end else begin
...
%000000                         expanded = {7'b0, rs2_5, rd_rs1_5, 3'b000, rd_rs1_5, `OPC_OP};
                            end
                        end
                    end
%000000             3'b110: begin
