...
%000000             if_ex_instr      <= 32'h0;
%000000             if_ex_pc         <= 32'h0;
%000000             if_ex_valid      <= 1'b0;
%000000             if_ex_pred_taken <= 1'b0;
%000000             if_ex_pred_ras   <= 1'b0;
%000000             if_ex_is_16bit   <= 1'b0;
...
%000001     wire        id_is_jal, id_is_jalr;
...
%000001     wire ex_actual_taken = if_ex_valid && (branch_taken | id_is_jal | id_is_jalr);
        
            // 只比 direction (1-bit XOR)：pred_taken=1 + actual=1 (any target) → no flush
%000001     wire ex_mispredict = if_ex_valid && (if_ex_pred_taken != ex_actual_taken);
...
%000001     wire        ex_bp_upd_taken  = ex_actual_taken;
...
%000000     wire wb_take_irq;                          // forward-declare (定義在後面)
%000000     wire wb_redirect = wb_take_irq || (ex_wb_valid_r && ex_wb_is_mret_r);
...
%000000         if (wb_take_irq) begin
%000000             pc_redirect     = 1'b1;
%000000             redirect_target = mtvec_o;
