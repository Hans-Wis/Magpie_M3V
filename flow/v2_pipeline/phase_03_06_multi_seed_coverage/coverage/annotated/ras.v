...
%000000     output     [31:0] ras_top,         // 目前 stack top (給 caller 在 IF 用)
        
            // ---- Push/Pop control (synchronous) ----
%000000     input             push,            // 1 = end of this cycle push
~000205     input      [31:0] push_val,
%000000     input             pop              // 1 = end of this cycle pop
        );
        
            localparam DEPTH = 8;
            localparam PTR_BITS = 3;
        
%000000     reg [31:0]         stack [0:DEPTH-1];
%000000     reg [PTR_BITS-1:0] ptr;            // points to *next empty slot* (=stack size)
...
%000000                     stack[0]       <= push_val;       // empty 時就視為 push 到 slot 0
                        // ptr 不變
