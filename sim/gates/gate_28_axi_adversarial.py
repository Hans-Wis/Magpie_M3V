...
NPU_RTL = [ROOT / f"design/npu/rtl/{m}.v" for m in ("npu_top", "npu_axil_regs", "npu_dma", "npu_tcm", "axil_decerr", "mat_engine", "npu_ml_ctrl", "npu_strip_buf")]
NPU_RTL += CPU_M1_RTL  # ADR-0034: npu_top now instantiates the cpu_m1 sequencer
MEM = ROOT / "design/npu/dv/tb/axi_full_mem.v"


@pytest.mark.skipif(not shutil.which("verilator"), reason="no verilator — not-run")
def test_adversarial_axi_lite_and_dma_ctrl(tmp_path):
    files = NPU_RTL + [MEM, ROOT / "design/npu/dv/tb/tb_axi_adversarial.v"]
...
def test_dma_flags_read_slverr(tmp_path):
    files = [ROOT / "design/npu/rtl/npu_dma.v", MEM, ROOT / "design/npu/dv/tb/tb_dma_err.v"]
