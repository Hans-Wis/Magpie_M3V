...
vlseg<nf>e<eew>.v / vsseg<nf>e<eew>.v, scope: nf=2..8, EEW=SEW, LMUL m1/m2/m4 (nf*L<=8), unmasked, vstart=0, unit-stride. element-major/field-minor interleave:
...
beat-by-beat, and (LMUL>1) into nf*L-register groups with a per-register tail blend.
Out-of-scope (EEW!=SEW / masked / m8 / unaligned vd) is illegal. vlseg2e8@m2 (the
compiler's actual emission) is covered.
...
def test_e2_corpus_covers_seg_forms():
    ...
    assert "e8, m2" in fw, "E2 must cover the compiler LMUL=m2 emission"
