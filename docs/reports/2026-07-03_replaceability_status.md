...
| # | 面向 | 狀態 | 證據 / 缺口 |
|---|---|---|---|
| 1 | 純量 ISA(RV32IMF_Zbb) | 🟢 GREEN(2026-07-05,F 完整) | **ADR-0050 F1-F4**:fexu 全套(moves/compare/classify/cvt + fadd/fsub/fmul + 4×FMA + fdiv/fsqrt,softfloat-3 忠實轉寫)+ 完整 fcsr/FS 契約;F-reg 寫入 lockstep 逐筆位元比對(rd=64+f)。證據 gate_60/61:directed 135+269、random 5 seeds ~5,524 commits 對 Spike rv32imf 全符(全 rm×NaN×∞×subnormal×OF/UF/DZ/NV)。F4 為組合邏輯(多拍化=Phase 7,ADR 已記偏離) |
