"""gate_p11_csr - P11: csr unit coverage gate (Tier-2 Industrial) — hardest stateful, FSM + waivers.

4-agent loop: Grok charter + Gemini toggle enumeration -> Codex tb_csr_unit.v + Verilator + VCS/URG
(-cm fsm) -> Claude independently verifies waiver basis vs csr.v RTL, approves, accepts. Tier-2: line
100, branch 100, expr >=95, toggle >=95, FSM state+arc 100 — after JUSTIFIED structural waivers
(design/cpu_m1/dv/cov/waivers/P11_csr.json): counter cycle/instret[27:63], mstatus/mie/mip WPRI-reserved,
...
PHASE = ROOT / "flow/v2_pipeline/phase_p11_csr"
COV = ROOT / "design/cpu_m1/dv/cov"
WAIVER = COV / "waivers/P11_csr.json"
MODULE = "csr"

pytestmark = pytest.mark.skipif(
    not (PHASE / "coverage/coverage.dat").exists(), reason="P11 csr phase not yet run")
...
def test_p11_line_toggle_tier2_adjusted():
    ...
    ln = rep["line"]["per_module"]["csr.v"]
    tg = rep["toggle"]["per_module"]["csr.v"]
...
def test_p11_branch_expr_fsm_tier2_via_vcs():
    cm = _cm()
...
def test_p11_waivers_approved_and_structural():
