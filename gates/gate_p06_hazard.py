"""gate_p06_hazard - P06: hazard unit coverage gate (Tier-2 Industrial).

Leaf unit gate (template = gate_p02_alu). 4-agent loop: Grok charter (.run/p04_08/grok_charters.log
P06 section; funct3-default coverable by illegal-funct3 vector, not a waiver) -> Codex implemented
tb_hazard_unit.v + Verilator + VCS/URG -> Claude verifies via cov_metrics + parse_urg and accepts.
...
pytestmark = pytest.mark.skipif(
    ...
    reason="P06 forward phase not yet run (coverage absent) — gate is not-run until Codex produces coverage")
...
def test_p04_golden_check_passed():
    ...
    m = re.search(r"PASS: hazard unit (\d+)/(\d+) vectors", logs)
    assert m and int(m.group(1)) == int(m.group(2)) and int(m.group(1)) >= 30, "hazard golden short/failed"
...
def test_p06_branch_and_expr_meet_tier2_via_vcs():
    cm = _cm()
