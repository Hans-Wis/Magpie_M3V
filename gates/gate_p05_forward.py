"""gate_p05_forward - P05: forward unit coverage gate (Tier-2 Industrial).

Leaf unit gate (template = gate_p02_alu). 4-agent loop: Grok charter (.run/p04_08/grok_charters.log
P05 section; funct3-default coverable by illegal-funct3 vector, not a waiver) -> Codex implemented
tb_forward_unit.v + Verilator + VCS/URG -> Claude verifies via cov_metrics + parse_urg and accepts.
...
PHASE = ROOT / "flow/v2_pipeline/phase_p05_forward"
...
pytestmark = pytest.mark.skipif(
    ...
    reason="P05 forward phase not yet run (coverage absent) — gate is not-run until Codex produces coverage")
...
def test_p04_artifacts_exist():
    for p in ["../../../design/cpu_m1/dv/tb/tb_forward_unit.v", "coverage/coverage.dat",
              "coverage/coverage.info", "vcs/urgReport/mod0.html"]:
        f = (PHASE / p).resolve()
        assert f.exists() and f.stat().st_size > 0, f"missing P05 artifact: {p}"


def test_p04_golden_check_passed():
    logs = " ".join(p.read_text(errors="replace") for p in (PHASE / "logs").glob("*.log"))
    m = re.search(r"PASS: forward unit (\d+)/(\d+) vectors", logs)
    assert m and int(m.group(1)) == int(m.group(2)) and int(m.group(1)) >= 30, "forward golden short/failed"
