"""gate_p07_mul - P07: mul unit coverage gate (Tier-2 Industrial).

Leaf unit gate (template = gate_p02_alu). 4-agent loop: Grok charter (.run/p04_08/grok_charters.log
P07 section; funct3-default coverable by illegal-funct3 vector, not a waiver) -> Codex implemented
tb_mul_unit.v + Verilator + VCS/URG -> Claude verifies via cov_metrics + parse_urg and accepts.
...
PHASE = ROOT / "flow/v2_pipeline/phase_p07_mul"
...
pytestmark = pytest.mark.skipif(
    ...
    reason="P07 forward phase not yet run (coverage absent) — gate is not-run until Codex produces coverage")
...
def test_p04_artifacts_exist():
    for p in ["../../../design/cpu_m1/dv/tb/tb_mul_unit.v", "coverage/coverage.dat",
              "coverage/coverage.info", "vcs/urgReport/mod0.html"]:
        f = (PHASE / p).resolve()
        assert f.exists() and f.stat().st_size > 0, f"missing P07 artifact: {p}"


def test_p04_golden_check_passed():
    logs = " ".join(p.read_text(errors="replace") for p in (PHASE / "logs").glob("*.log"))
    m = re.search(r"PASS: mul unit (\d+)/(\d+) vectors", logs)
    assert m and int(m.group(1)) == int(m.group(2)) and int(m.group(1)) >= 30, "mul golden short/failed"
...
def test_p07_branch_and_expr_meet_tier2_via_vcs():
    cm = _cm()
    urg = cm.parse_urg(str(PHASE / "vcs/urgReport"), module=MODULE)
    # branch/expr are N/A if the module has no such coverage points (e.g. pure boolean assigns).
    if "branch" in urg:
        assert urg["branch"]["pct"] == 100.0, f"branch {urg['branch']} < 100% Tier-2"
    if "expr" in urg:
        assert urg["expr"]["pct"] >= 95.0, f"expr {urg['expr']} < 95% Tier-2"
    assert "branch" in urg or "expr" in urg, "VCS produced no branch/expr coverage at all"
