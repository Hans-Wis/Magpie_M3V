"""gate_p13_bp - P13: bp unit coverage gate (Tier-2 Industrial) — hardest stateful, FSM + waivers.

4-agent loop: Grok charter + Gemini toggle enumeration -> Codex tb_csr_unit.v + Verilator + VCS/URG
(-cm fsm) -> Claude independently verifies waiver basis vs bp.v RTL, approves, accepts. Tier-2: line
100, branch 100, expr >=95, toggle >=95, FSM state+arc 100 — after JUSTIFIED structural waivers
(design/cpu_m1/dv/cov/waivers/P13_bp.json): counter cycle/instret[27:63], mstatus/mie/mip WPRI-reserved,
...
PHASE = ROOT / "flow/v2_pipeline/phase_p13_bp"
COV = ROOT / "design/cpu_m1/dv/cov"
WAIVER = COV / "waivers/P13_bp.json"
MODULE = "bp"

pytestmark = pytest.mark.skipif(
    not (PHASE / "coverage/coverage.dat").exists(), reason="P13 csr phase not yet run")
...
def test_p13_line_toggle_tier2_adjusted():
    ...
    ln = rep["line"]["per_module"]["bp.v"]
    tg = rep["toggle"]["per_module"]["bp.v"]
...
def test_p13_branch_expr_fsm_tier2_via_vcs():
    cm = _cm()
...
def test_p13_waivers_approved_and_structural():
