...
OPCODES = {'MAT_CFG': 1, 'MAT_LOAD_W': 2, 'MAT_OP': 3, 'MAT_RESCALE': 4, 'MAT_STORE': 5, 'MAT_ACC_CLR': 6, 'MAT_FENCE': 7, 'MAT_ACT_LUT': 8, 'MAT_EWISE_MUL': 9, 'MAT_RMSNORM': 10, 'MAT_EWISE_ADD_REQUANT': 11, 'MAT_ROPE': 12, 'MAT_SOFTMAX': 13, 'MAT_REQUANT_VEC': 14, 'MAT_STRIP_GEMM': 15}
OPCODE_NAMES = {v: k for k, v in OPCODES.items()}
ERR_CAUSES = {'BAD_OPCODE': 1, 'RSVD_VIOLATION': 2, 'RING_OVERRUN': 3, 'ENGINE_NOT_READY': 4, 'DMA_FAULT': 5, 'DESC_ALIGN': 6, 'MAT_PARAM': 7, 'ABORTED': 8, 'STRIP': 9, 'CORE_TRAP_FLAG': 2147483648}
...
def encode(op, **fields):
    ...
    if opv == OPCODES['MAT_RMSNORM']:
        return [w0, fields.get('src', 0) & 0xFFFFFFFF, fields.get('dst', 0) & 0xFFFFFFFF, fields.get('param', 0) & 0xFFFFFFFF]
    if opv == OPCODES['MAT_EWISE_ADD_REQUANT']:
    ...
    if opv == OPCODES['MAT_REQUANT_VEC']:
        return [w0, fields.get('mult_q31', 0) & 0xFFFFFFFF, ((fields.get('rsp', 0) & 0xFFFF) << 16) | (fields.get('clamp', 0) & 0xFFFF), ((fields.get('src', 0) & 0xFFFF) << 16) | (fields.get('dst', 0) & 0xFFFF)]
    if opv == OPCODES['MAT_STRIP_GEMM']:
        return [w0, fields.get('w_base', 0) & 0xFFFFFFFF, ((fields.get('n_strips', 0) & 0xFFF) << 17) | (fields.get('strip_bytes', 0) & 0x1FFFF), ((fields.get('k_chunks', 0) & 0xFF) << 24) | ((fields.get('k_tail', 0) & 0x7F) << 17) | ((fields.get('n_tail', 0) & 0x7F) << 10) | (fields.get('out_dst64', 0) & 0x3FF)]
    raise ValueError('bad opcode %r' % (op,))
...
def decode(words):
    ...
    elif opv == OPCODES['MAT_RMSNORM']:
        d.update({'src': w1, 'dst': w2, 'param': w3, 'len': d['rpt']})
    elif opv == OPCODES['MAT_EWISE_ADD_REQUANT']:
    ...
    elif opv == OPCODES['MAT_REQUANT_VEC']:
        d.update({'src': (w3 >> 16) & 0xFFFF, 'dst': w3 & 0xFFFF, 'mult_q31': w1, 'rsp': (w2 >> 16) & 0xFFFF, 'clamp': w2 & 0xFFFF, 'len': d['rpt']})
    elif opv == OPCODES['MAT_STRIP_GEMM']:
        d.update({'w_base': w1, 'strip_bytes': w2 & 0x1FFFF, 'n_strips': (w2 >> 17) & 0xFFF, 'k_chunks': (w3 >> 24) & 0xFF, 'k_tail': (w3 >> 17) & 0x7F, 'n_tail': (w3 >> 10) & 0x7F, 'out_dst64': w3 & 0x3FF})
    return d
...
def _self_test():
    samples = {
        ...
        'MAT_REQUANT_VEC': dict(acc=1, mult_q31=0x40000000, rsp=(0<<8)|38, clamp=(127<<8)|0x80, src=0x300, dst=0x400, rpt=64),
        'MAT_STRIP_GEMM': dict(w_base=0x4000, strip_bytes=4096, n_strips=1, k_chunks=1, k_tail=64, n_tail=64, out_dst64=0x60),
    }
