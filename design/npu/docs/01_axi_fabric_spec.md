...
The frozen `axil_bridge.v` (in `design/cpu_m1/rtl/`, untouched) already exposes the host cpu_m1 as
...
cpu_m1 (frozen) ── axil_bridge.v ──> m_axi_d_* (AXI4-Lite master, single-outstanding)
                                          │
                                    axil_1to2.v  (NEW, design/npu/rtl)
...
| file | role |
|---|---|
| `design/npu/rtl/axil_1to2.v` | 1-master → 2-slave AXI4-Lite router by address (single-outstanding, matches host bridge) |
| `design/npu/rtl/npu_axil_regs.v` | NPU AXI4-Lite **slave** CSR bank; drives `npu_start`/`npu_config`, samples `npu_busy`/`npu_done` |
| `design/npu/dv/tb/axil_mem16.v` | passthrough mem slave (stands in for host subsystem) |
| `design/npu/dv/tb/tb_axil_fabric.v` | host-master BFM + transaction scoreboard (8 checks) |
...
**Freeze guard** (`gate_10_host_noregress`): `design/cpu_m1/rtl/` must stay byte-identical to
